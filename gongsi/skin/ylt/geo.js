...
function changef(v) {
    var str = "0";
    for (i = 0; i < v; i++) { str += ("_" + (document.getElementById(f[i]).selectedIndex - 1));}
    ;
    var ff = document.getElementById(f[v]);
    with (ff) {
        length = 0;
        options[0] = new Option(opt0[v], opt0[v]);
        if (v && document.getElementById(f[v - 1]).selectedIndex > 0 || !v) {
            if (dsy.Exists(str)) {
                ar = dsy.Items[str];
                for (i = 0; i < ar.length; i++)options[length] = new Option(ar[i], ar[i]);
                if (v)options[0].selected = true;
...
function setupf() {
    for (i = 0; i < f.length - 1; i++)
        document.getElementById(f[i]).onchange = new Function("changef(" + (i + 1) + ");");
    changef(0);
}