...
$username2=md5(urlencode($username));
...
if($userid) {
	...
	if(is_file($img)){ $file = $img;$havepic=true;}
} else if($username) {
	$md5 = md5($username);
	$img = $DT_ROOT.'file/avatar/'.substr($md5, 0, 2).'/'.substr($md5, 2, 2).'/_'.$username.$ext;
	if(is_file($img) && preg_match("/^[a-z0-9]{1}[a-z0-9_\-]{0,}[a-z0-9]{1}$/", $username)) {$file = $img;$havepic=true;}
}
if($size == 'large'&&$username&&!$havepic){$ext = 'pic/'.substr($username2,-1,1).'.jpg';$file = $DT_ROOT.'api/avatar/default'.$ext;;}
//echo $file;
