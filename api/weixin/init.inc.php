...
class weixin {
	...
	function get_ticket($access_token){	
		...
		// 	$arr = $this->http_get($url);
		// 	$ticket = $arr['ticket'];
		// 	if ($ticket) $dc->set('weixin_ticket', $ticket, 7000);
		// }
		// return $ticket;
	...
	//生成JS-SDK权限验证签名--吕保成--2016/2/1
	function get_wxconfig($ticket, $url=''){
...
//获取ticket--吕保成--2016/1/26
$ticket = $dc->get('weixin_ticket');//ticket
if(!$ticket) {
	$ticket = $wx->get_ticket($wx->access_token);
	$dc->set('weixin_ticket', $ticket, 7000);
}
$wx->ticket = $ticket;

//获取wxconfig--吕保成--2016/2/2
$wxconfig = $wx->get_wxconfig($wx->ticket);
