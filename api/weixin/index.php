		...
		if($MsgType == 'event') {//事件
			...
			switch($MsgType) {
				case 'text'://文本消息
					...
					//查找关键词规则
					$reply_rule = new tcdb('weixin_reply');
					$keyword_content = '"'.$Content.'"';
					//关键词回复
					$rule = $reply_rule->likeWhere(['keyword'=>$keyword_content])->where(['status'=>3])->order('itemid desc')->one();
					if(!empty($rule) && !empty($rule['content'])){
						...
						$weixin_chat = new tcdb('weixin_chat');
						$user_chat = $weixin_chat->where(['openid'=>$FromUserName])->where(['addtime'=>$half_hour_age],'>')->one(); 
						if(empty($user_chat)){		//半个小时内，用户第一次发送信息，对用户发送自动回复内容
							$auto_rule = $reply_rule->where(['type'=>1])->order('itemid desc')->one();
							if(!empty($auto_rule) && !empty($auto_rule['content'])) {
								$reply = str_replace('<br />',"\n",$auto_rule['content']);
								$reply = str_replace('&nbsp;',' ',$reply);
								$wx->response($FromUserName,$ToUserName,'text',$reply,$misc);	//回复内容
							}
