...
$_SESSION['authorize_login_forother'] = 'other';
if($success) {
	$U = array();
	...
	if($_userid) {
		...
		if($U['username']) {
			...
			if($user) {
				...
				if($_SESSION['authorize_login'] && $_SESSION['authorize_login_forother']){
					$_SESSION['authorize_login'] = '';
					$_SESSION['authorize_login_forother'] = '';
					include template('oauth_login','member/oauth');
				}
				dheader($forward);
			} else {
			...
			if($U['site']=='wechat') set_cookie('weixin_openid', encrypt($U['openid'], DT_KEY.'WXID'));
