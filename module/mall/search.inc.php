...
$malls = $mall_db->field('itemid,title,company,linkurl,thumb,catid,areaid,kcatids,username')
    ->order('itemid desc')->limit($start, $pagesize)->select();
$scws = new scws();
$stopWord = $scws->getStopWord();
foreach($malls as $key => $v){
    $malls[$key]['title'] = str_replace($stopWord,'*',$v['title']);
}
$items = $mall_db->where($condition_str)->count('c');

//推荐产品
$rmalls = $mall_db->field('itemid,thumb,title,kcatids,linkurl,company,username')
    ->where(['status' => 3])->where($rinCondition, 'in')->order('hits desc')->limit(0, 6)->select();
//猜你喜欢关键词
