    ...
    case 'add':
        ...
        if ($submit) {
            ...
            if ($do->pass($post)) {
                ...
                $numa = $db->count($DT_PRE . 'mall', "username='" . $_username . "'", 60);
                $numa = $numa + 1;
                $db->query("UPDATE tc_company SET pnum=$numa WHERE username='" . $_username . "'");

                if ($_username = 'tiancheng') {
                    $post['level'] = 1;
                }                //--2016/3/14/chentao

                //--2015/9/1/chentao--start
                if (check_dingzhi_member($_username, 'section_price') && $post_fields['issectionprice'] == '1') {       //定制用户价格区间产品添加
                    $section = json_decode(str_replace('\"', '"', $post_fields['sectionprice']), true);
                    foreach ($section as $k => $v) {              //批量添加商品
                        $p = $post;
                        $p['price'] = $v;
                        $do->add($p);
                        $pf = $post_fields;
                        $pf['sectionprice'] = $k;
                        if ($FD) fields_update($pf, $table, $do->itemid);
            ...
            $hasProductLicense = $companyModule->hasProductLicense($_userid);
