...
class brand {
	...
	function edit($post) {
		...
		$sql="UPDATE {$this->table} SET $sql WHERE itemid=$this->itemid";
			//print_r($sql);exit;
	    $this->db->query($sql);
		$content_table = content_table($this->moduleid, $this->itemid, $this->split, $this->table_data);
		$this->db->query("REPLACE INTO {$content_table} (itemid,content) VALUES ('$this->itemid', '$post[content]')");
		$this->update($this->itemid,'edit');
		clear_upload($post['content'].$post['thumb'], $this->itemid);
	...
	function update2($itemid,$type='contact') {
		...
		if($member&&$member['groupid']>5) {
			foreach(array('groupid','vip','validated','company','areaid','truename','telephone','mobile','address','qq','msn','ali','skype') as $v) {
				if($item[$v]== '') $update .= ",$v='".addslashes($member[$v])."'";
			}
			if($item['email'] != $member['mail']) $update .= ",email='".addslashes($member['mail'])."'";
		}
            global $db;
            $sql="SELECT {$db->pre}company.*,{$db->pre}company_data.content FROM {$db->pre}company join {$db->pre}company_data WHERE {$db->pre}company.userid={$db->pre}company_data.userid and {$db->pre}company.company='$title'";
            $c = $this->db->get_one($sql);

            $sql="SELECT {$db->pre}brand_data_13.content FROM {$db->pre}brand_data_13 WHERE {$db->pre}brand_data_13.itemid=$itemid";
            $b = $this->db->get_one($sql);   

            $sql="SELECT COUNT(*) AS num FROM {$db->pre}mall WHERE status=3 and factoryid=$itemid";
            $p = $this->db->get_one($sql);   
            $pnum=$p['num'];
			$update.=",pnum=".$pnum;
			if($c&&$contact=='contact')
			{
			$thumb=$item['thumb'];$content=strip_nr($b['content'],1);
			if($item['thumb']==''&&$c['thumb']!=''){$thumb=$c['thumb'];}
			if(strlen($b['content'])<strlen($c['content']))
			  {
			  $content=strip_nr(strip_tags($c['content']),1);
			  $introduce=dsubstr($content,100,'..');
			  }
			$this->db->query("UPDATE {$db->pre}brand_13 SET thumb='$thumb',introduce='$introduce' WHERE itemid=$itemid");
			$this->db->query("UPDATE {$db->pre}brand_data_13 SET content='$content' WHERE itemid=$itemid");
			}
         $update="UPDATE {$this->table} SET ".(substr($update, 1))." WHERE itemid=$itemid";
        //print_r($update);exit;
