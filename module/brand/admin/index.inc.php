...
if(in_array($action, array('', 'check', 'expire', 'reject', 'recycle'))) {
	...
	$sorder  = array('结果排序方式', '更新时间降序', '更新时间升序', '添加时间降序', '添加时间升序', '浏览次数降序', '浏览次数升序', '品牌ID降序', '品牌ID升序', '产品数降序');
	$dorder  = array($MOD['order'], 'edittime DESC', 'edittime ASC', 'addtime DESC', 'addtime ASC', 'hits DESC', 'hits ASC', 'itemid DESC', 'itemid ASC', 'pnum desc');
	...
	if($homepages==1) $condition .= " AND homepage<>''";
	if($homepages==2) $condition .= " AND homepage=''";
		...
		if($submit) {
			if($do->pass($post)) {
				...
				$do->edit($post);
					$title=$post['title'];
					$fid=$itemid;
					$k = $db->get_one("SELECT COUNT(*) AS num FROM {$db->pre}mall WHERE status=3 and factoryid=".$fid);
					$db->query("UPDATE {$db->pre}brand_13 SET pnum=".$k['num']." WHERE itemid=".$fid);
					$db->query("UPDATE {$db->pre}mall SET factory='".$title."',brand='".$title."' WHERE factoryid=".$fid." and factory<>'".$title."'");
					$db->query("UPDATE {$db->pre}company SET brandid=',".$fid.",' WHERE company='".$title."'");
				dmsg('修改成功', $forward);
			} else {
				msg($do->errmsg);
			}
