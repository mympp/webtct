...
class buy {
	...
	function get_one() {//V5.0 升级 V6.0 修改
	...
	function get_purlist($condition = 'status>0', $order = 'edittime DESC', $cache = '') {//TC添加函数
		global $MOD, $pages, $page, $pagesize, $offset, $items, $sum;
		if($page > 1 && $sum) {
			$items = $sum;
		} else {
			$r = $this->db->get_one("SELECT COUNT(*) AS num FROM {$this->db->pre}buy_pur_6 WHERE $condition", $cache);
			$items = $r['num'];
		}
		$pages = defined('CATID') ? listpages(1, CATID, $items, $page, $pagesize, 10, $MOD['linkurl']) : pages($items, $page, $pagesize);
		$lists = $catids = $CATS = array();
		$result = $this->db->query("SELECT * FROM {$this->db->pre}buy_pur_6 WHERE $condition ORDER BY itemid desc LIMIT $offset,$pagesize", $cache);
		while($r = $this->db->fetch_array($result)) {
			$r['alt'] = $r['title'];
			$r['title'] = set_style($r['title'], $r['style']);
			$r['linkurl'] = $MOD['linkurl'].$r['linkurl'];
	...
	function add($post) {
		...
		//只有添加的时候生成采购编�?
			$r = $this->db->get_one("SELECT catdir FROM {$this->db->pre}category WHERE catid=".$post['catid']);
			$catdir = $r['catdir'];
			$this->db->query("UPDATE  {$this->table} SET buycode='cgzb-".$catdir."-".$this->itemid."' WHERE itemid=$this->itemid");
		//只有添加的时候生成采购编�?
	...
	function special($itemid) {//TC添加函数
		$d = $this->db->get_one("SELECT title FROM {$this->db->pre}special WHERE itemid=$itemid");
		return $d['title'];
	}
	...
	function update($itemid) {
		...
		if($member&&$member['groupid']>5) $update .= update_user($member, $item);//V5.0 升级 V6.0 TC修改过
	...
	function check($itemid) {
		...
		if(is_array($itemid)) {
			...
			#审核通过招标发�?邀请函通知供应�?
			$getMessage = $this->db->query("SELECT * FROM {$this->db->pre}message WHERE groupids = $itemid");
			if($getMessage){
				while($r = $this->db->fetch_array($getMessage)){
					$this->db->query("UPDATE {$this->db->pre}member SET message=message+1 WHERE username='$r[touser]'");
				}
				$this->db->query("UPDATE {$this->db->pre}message SET status=3,groupids='' WHERE groupids = $itemid");
			}
			#审核通过招标发�?邀请函通知供应�?
			$this->tohtml($itemid);
			return true;
