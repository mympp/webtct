...
switch($action) {
	...
	case 'edit':
		...
		if($submit) {
			if($do->vip_edit($vip)) {
				//--2016/3/7/chentao--start
				if($vip['promotion']){
					$vip['promotion']=1;
				}else{
					$vip['promotion']=0;
				}
				$db->query("update {$db->pre}member set promotion=".$vip['promotion']." where userid=".$do->userid);
				//--2016/3/7/chentao--end				
				dmsg('修改成功', $forward);
			} else {
				msg($do->errmsg);
			}
	...
	case 'delete':
		...
		$user_id.=')';
		$db->query("update {$db->pre}member set promotion = 0 where userid in ".$user_id);        
		//--2016/3/7/chentao--end
		dmsg('撤销成功', $forward);
	break;
	case 'update':
