...
class company {
	...
	function update2($userid) {//TC添加函数
		global $DT, $MOD;
		global $db;
		$this->userid = $userid;
		$r = $this->get_one();
		...
		if($r['vip']) {
            ...
            $sql="SELECT {$db->pre}company_data.content FROM {$db->pre}company_data WHERE {$db->pre}company_data.userid=$userid";
            $c = $this->db->get_one($sql);
			if(!$c){$db->query("INSERT INTO {$db->pre}company_data (content,userid) VALUES('$keyword','$userid')");}

            $sql="SELECT {$db->pre}company.*,{$db->pre}company_data.content FROM {$db->pre}company join {$db->pre}company_data WHERE {$db->pre}company.userid={$db->pre}company_data.userid and {$db->pre}company.userid=$userid";
            $c = $this->db->get_one($sql);

            $sql="SELECT {$db->pre}brand_13.*,{$db->pre}brand_data_13.content FROM {$db->pre}brand_13 join {$db->pre}brand_data_13 WHERE {$db->pre}brand_13.itemid={$db->pre}brand_data_13.itemid and {$db->pre}brand_13.title='".$c['company']."'";
            $b = $this->db->get_one($sql);   

            $sql="SELECT COUNT(*) AS num FROM {$db->pre}mall WHERE status=3 and username='".$r['username']."'";
			...
			$closeshop=$c['closeshop'];
			$pnum=$p['num'];
			if($c['type']==''){$type='企业单位';}else{$type=$c['type'];}
			if($c['mode']==''){$mode='贸易商,服务商';}else{$mode=$c['mode'];}
			if($c['business']==''){$business=$c['title'];}else{$business=$c['business'];}
			if($c['thumb']==''&&$b['thumb']!=''){$thumb=$b['thumb'];$brandid=','.$b['itemid'].',';}
			if($template==''){$template='homepage';$skin='default';}
			if($mail&$thumb){$closeshop=0;}
			if($c['homepage']==''&&$b['homepage']!=''){$homepage=$b['homepage'];}
			if(strlen($c['content'])<strlen($b['content']))
			  {
			  $content=strip_nr(strip_tags($b['content']),1);
			  $introduce=dsubstr($content,100,'..');
			  }
			$this->db->query("UPDATE {$db->pre}company SET thumb='$thumb',introduce='$introduce',type='$type',brandid='$brandid',mode='$mode',homepage='$homepage',mail='$mail',telephone='$telephone',template='$template',skin='$skin',closeshop='$closeshop',pnum='$pnum' WHERE userid=$userid");
			$this->db->query("UPDATE {$db->pre}company_data SET content='$content'  WHERE userid=$userid");
			}
