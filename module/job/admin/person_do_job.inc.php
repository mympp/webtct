...
switch($action){
	...
	case 'product_comment_pingjia':
		...
		include tpl('person_do_product_comment_pingjia',$module);
	break;
	case 'person_do_resume':
		$resume = $do->get_resume_list();
		include tpl('person_do_resume',$module);
	break;
	case 'person_do_resume_comment':
		$comments = $do->person_do_resume_comment();
		$members = $do->person_do_resume_sel_member();
		include tpl('person_do_resume_comment',$module);
	break;
	case 'person_do_resume_commented':
		if($do->check_empty($post["star_attitude"])){
			if($do->check_empty($post["star_time"])){
				if($do->check_empty($post["star_quality"])){
					if($do->check_empty($post["seled_members"])){
						if($do->check_empty($post["seled_comments"])){
							$do->resume_commented($post);
							dmsg("人气点评成功！",$forward);
						}else{
							msg("请选择评语！");
						}
