...
$ad = $db->get_one("SELECT * FROM {$DT_PRE}ad WHERE pid=$p[pid] AND status=3 AND fromtime<$DT_TIME AND totime>$DT_TIME AND areaid=$areaid ORDER BY fromtime DESC");
			if($ads>1){
				//多图片广告随机播放效果，黄浩彬，2015/12/24
				$adl = $db->query("SELECT * FROM {$DT_PRE}ad WHERE pid=$p[pid] AND status=3 AND fromtime<$DT_TIME AND totime>$DT_TIME AND areaid=$areaid ORDER BY fromtime DESC ");
					$adm=$m=array();
					while($t = $db->fetch_array($adl)) {
						if($t['url']) $m['url']=$t['url'];
							if($t['stat']) {
								$m['url'] = DT_PATH.'api/'.rewrite('redirect.php?aid='.$t['aid']);
							} else {
								if(strpos($m['url'], '://') === false) $m['url'] = $MODULE[$ad_moduleid]['linkurl'].$t['linkurl'];
							}
						if(strpos($t['image_src'], '://') === false) $t['image_src'] = DT_PATH.$t['image_src'];
						$m['pic']=$t['image_src'];
						$adm[]=$m;
