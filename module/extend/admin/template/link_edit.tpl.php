...
<td class="tl"><span class="f_red">*</span>链接模块</td>
<td>
	<select name="post[link_moduleid]" id="select-moduleid">
		<option value="0">请选择模块</option>
		<?php
			foreach ($modules as $key => $module) {
				echo '<option value="'.$module['moduleid'].'">'.$module['name'].'</option>';
			}
		?>
	</select>
	
</td>
<?php if (isset($link_moduleid)) {
?>
	<script type="text/javascript">
	$(function(){
		$("#select-moduleid").val('<?php echo $link_moduleid;?>');
	});
	</script>
<?php
}?>
