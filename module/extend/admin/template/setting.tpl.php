...
?>
    <form method="post" action="?">
        ...
        <table cellpadding="2" cellspacing="1" class="tb">
            ...
            <tr>
                <td class="tl">全站更新</td>
                <td><?php echo timetodate(filemtime(DT_ROOT.'/sitemaps.xml'));?>&nbsp;&nbsp; <a href="?moduleid=<?php echo $moduleid;?>&file=sitemap&tn=yes&action=sitemaps" class="t" target="send">立即更新</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="?moduleid=<?php echo $moduleid;?>&file=sitemap&action=sitemaps&ys=yes" class="t" target="_blank">原生SITEMAP</a></td>
            </tr>
            <tr>
                <td class="tl">模块更新</td>
                <td>
                    更新列表sitemap：<a href="?moduleid=3&file=listsitemap&action=list&m=company" class="t" onclick="this.style.color='red'">企业</a>
                    &nbsp;&nbsp;
                    <a href="?moduleid=3&file=listsitemap&action=list&m=mall" class="t" onclick="this.style.color='red'">产品</a>
                    &nbsp;&nbsp;
...
=======
                    &nbsp;&nbsp;
                    <a href="?moduleid=3&file=listsitemap&action=list&m=mallArticle" class="t" onclick="this.style.color='red'" >产品知识库</a>

>>>>>>> .r3010
                    <br/>
                    <iframe src="" name="send" id="send" style="display:none;" scrolling="no" frameborder="0"></iframe>
                    <?php foreach($mods as $md){?>
                        <a href="?moduleid=<?php echo $moduleid;?>&file=sitemap&tn=yes&action=sitemaps&mods=<?php echo $md?>" class="t" onclick="this.style.color='red'" target="send"><?php echo $MODULE[$md]['name']?>

                        <?php if($md==4){
                            $m = $db->get_one("SELECT count(*) as num FROM ".$DT_PRE."company WHERE catids<>'' and closeshop=0 and groupid>1");
                        }else{
                            $tables=get_table($md);
                            if(strlen($tables)>4){
                                $m = $db->get_one("SELECT count(*) as num FROM ".$tables." WHERE status>2");
                            }
                            $modnum=$m['num'];
                            $j = ceil($modnum/$sitemaps_items);
                            echo $modnum.'/'.$j;?>
                            :</a>
                            <?php
                            for($i=1;$i<=$j;$i++){
                                echo "<a href='?moduleid=".$moduleid."&file=sitemap&tn=yes&action=sitemaps&mods=".$md."&i=".$i."'  onclick=\"this.style.color='red'\" target=\"send\">[".$i."]</a>";
                            }
                        }
                        ?>
                        ...
                        <br>
                    <?php }?>
                    <a href="?moduleid=<?php echo $moduleid;?>&file=sitemap&tn=yes&action=sitemaps&mods=9.5" class="t" onclick="this.style.color='red'"  target="send">技术工程师
                        <?php $m = $db->get_one("SELECT count(*) as num FROM ".$DT_PRE."resume WHERE status>2");$modnum=$m['num'];$j = ceil($modnum/$sitemaps_items);echo $modnum.'/'.$j;?>:
                    </a>
                    <?php
                    for($i=1;$i<=$j;$i++){
                        echo "<a href='?moduleid=".$moduleid."&file=sitemap&tn=yes&action=sitemaps&mods=9.5&i=".$i."'  onclick=\"this.style.color='red'\" target=\"send\">[".$i."]</a>";
                    }
                    ?>
                    <br>
                    <br>
                    <a href="?moduleid=<?php echo $moduleid;?>&file=sitemap&tn=yes&action=sitemaps&all=yes" class="t" target="send" onclick="this.style.color='red'" >全站SITEMAP</a>
                </td>
            </tr>
            <tr>
