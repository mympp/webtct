...
/*
		...
		人物：李通
		事件：
		关联：module/admin/template/tctuijian.tpl.php
		mysql
*/
case 'tctuijian';
		$do->itemid = $itemid;
		if($submit) {
			...
			$a = $db->get_one("SELECT * FROM `{$DT_PRE}category` WHERE catid=$catid");
			$catname = $a[catname];
			$db->query("UPDATE `{$DT_PRE}category` SET otheritem=otheritem+1 WHERE catid=$catid");
			$db->query("INSERT INTO `{$DT_PRE}photo_tj`(catid, catname,title, num2, num4,addtime) VALUES ('$catid','$catname','$title','$num2','$num4','$addtime')"); 
			dmsg('添加成功', '?moduleid='.$moduleid.'&action=tctuijian');
		}else {
...
#------end---------
	case 'tuijianlist';
		...
		if($condition==''){
				$r = $db->get_one("select count(DISTINCT itemid) as num from {$DT_PRE}photo_tj ");
			$items = $r['num'];
			$pages = pages($items, $page, $pagesize);		
			$child = array();
			$result = $db->query("SELECT * FROM {$DT_PRE}photo_tj ORDER BY itemid DESC LIMIT $offset,$pagesize");
			while($r = $db->fetch_array($result)) {
				
				$lists[] = $r;
			}
		}else{
			$result = $db->query("SELECT * FROM {$DT_PRE}photo_tj ORDER BY itemid DESC LIMIT 0,100");
			while($r = $db->fetch_array($result)) {
				$lists[] = $r;
			}
	...
	break;
	case 'deltuijian':
		$itemid or msg('请填写来源ID');
		$db->query("DELETE FROM `{$DT_PRE}photo_tj` WHERE `itemid`=$itemid");
		dmsg('删除成功', $forward);
	break;
	case 'edituijian':
		$itemid or msg('请填写来源ID');
