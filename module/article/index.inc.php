...
$links = $link_db->field('title,linkurl')->where(['status'=>3,'link_moduleid'=>21])->order('listorder asc')->limit(0,30)->select();

$top_news = getArticleTop();
if(!check_group($_groupid, $MOD['group_index'])) include load('403.inc');
$K = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}info_22");
$info22=$K['num'];
