...
<td><input type="text" size="60" name="member[company]" id="company" value="<?php echo $company;?>" onblur="validator('company');"/>&nbsp;<a href="#vv"><img src="<?php echo $MOD['linkurl'];?>image/<?php echo $vcompany ? 'v' : 'u';?>_company.gif" width="16" height="16" title="<?php echo $vcompany ? '已通过' : '未通过';?>工商认证" align="absmiddle"/></a>


<a href='http://www.baidu.com/baidu?tn=myie_dg&word=<?php echo $company?>' target="_blank"  title="百度搜他资料">搜资料</a>
</td>
</tr>
<tr>
<td class="tl">初始访问</td>
<td class="tr">
<input type="text" size="15" name="member[hits]" id="hits" value="<?php echo $hits;?>" maxlength="8" readonly="readonly"/>
</td>
</tr>
<tr>
...
<td>
<?php if(!$catids){?>
<?php $cates=',715,';$catids=$cates;$catid=$cates;?>
<?php }?>
...
<td><input type="text" size="80" name="member[business]" id="business" value="
<?php if($business){
echo $business;}
elseif($introduce){
if(strstr($introduce,'。'))
{echo dsubstr(str_replace('。','',strstr($introduce,'。')),80,'');}
else{echo dsubstr(strip_tags($introduce),80,'');}
...
<tr>
<?php if(!$sell){$sell=dsubstr(str_replace('，','|',str_replace('。','',strstr($introduce,'。'))),80,'');;}?>
...
<?php if(!$brandid){ ?>
<?php $r = $db->get_one("SELECT * FROM {$DT_PRE}brand_13 WHERE title='$company'");
$brandid=','.$r['itemid'].',';?>
<?php } ?>
...
<tr>
<td class="tl"><span class="f_hid">*</span> 移动端风格目录</td>
<td><input type="text" size="20" name="member[m_skin]" value="<?php echo $m_skin;?>"/></td>
</tr>
<tr>
<td class="tl"><span class="f_hid">*</span> 移动端模板目录</td>
<td><input type="text" size="20" name="member[m_template]" value="<?php echo $m_template;?>"/></td>
</tr>
<tr>
<td class="tl"><span class="f_hid">*</span> 主页模板目录</td>
<td><input type="text" size="20" name="member[template]"  ID="Mtemplate" value="<?php echo $template;?>"/>
<?php if($_userid==1){ ?>
<select name="post[skin]" id="skin" onchange="if(this.value){Dd('Mtemplate').value=this.value;}">
<option value="">子模板选择</option>
<?php } ?>
<?php
$d = DT_ROOT.'/template/tc/homepage/';
foreach(glob($d.'*') as $v) {
	if(is_dir($v)) {
		$n = basename($v);
		echo '<option value="homepage/'.$n.'">'.$n.'</option>';
	}
}
?>
<?php if($_userid==1){ ?>
