...
<?php
	...
	foreach($MODULE as $m) {
		if($m['moduleid'] > 4 && is_file(DT_ROOT.'/module/'.$m['module'].'/my.inc.php')) {
			if($m['moduleid'] == 9) {
				echo '<li><input type="checkbox" name="setting[moduleids][]" value="9" '.(in_array(9, $moduleids) ? 'checked' : '').' id="mod_9"/><label for="mod_9"> 服务需求</label></li>';
			...
			else if($m['moduleid'] == 28){
				echo '<li><input type="checkbox" name="setting[moduleids][]" value="28" '.(in_array(28, $moduleids) ? 'checked' : '').' id="mod_28"/><label for="mod_9">HR招聘</label></li>';
				echo '<li><input type="checkbox" name="setting[moduleids][]" value="-28" '.(in_array(-28, $moduleids) ? 'checked' : '').' id="mod__28"/><label for="mod__9">HR简历</label></li>';
			}
			else {
