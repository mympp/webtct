...
<td width="160"><?php if($v['catid']) { ?><?php echo $v['cate'];?><?php } else { ?>不限<?php } ?></td>
...
<?php if($v['sendtime']) { ?>
<td class="px11 f_gray"><font title="添加时间：<?php echo timetodate($v['addtime'], 5);?>"><?php echo timetodate($v['sendtime'], 3);?></font></td>
<?php } else { ?>
<td class="f_gray"><font title="添加时间：<?php echo timetodate($v['addtime'], 5);?>">从未</font></td>
<?php } ?>
...
<td>
<select name="sendtype" onchange="Go('?moduleid=2&file=alert&action=send&username=<?php echo $v['username'];?>&addtime='+this.value)">
	<option value="">选择发布</option>
	<option value="<?php echo $v['addtime'];?>">全部信息</option>
	<option value="<?php echo $v['sendtime'];?>">未发信息</option>
</select></td>
<td>
