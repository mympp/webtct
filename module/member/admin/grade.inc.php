...
switch ($action) {
    case 'allow':
        ...
        if($companyModule->allowUpgradeToCompany($itemid)){
            dmsg('操作成功', '?moduleid=2&file=grade');
        }else{
            msg('操作失败');
        }
        break;
    ...
    case 'reject':
        if($rejectMove){
            $itemid or msg();
            $companyModule = baseModule::moduleInstance('company');
            $note = empty($reason) ? '' : $reason;
            $result = $companyModule->rejectUpgradeToCompany($itemid,$note);
            if($result){
                dmsg('操作成功', '?moduleid=2&file=grade');
            }else{
                msg('操作失败');
            ...
            $result = $upgradeQuery->getListsByStatus(UpgradeQuery::REJECT_STATUS,$page,$pagesize);
            if($result){
                $lists = $result['lists'];
                $pages = pages($result['totalCount'],$page,$pagesize);
            }
    ...
    case 'check':
        $upgradeQuery = new UpgradeQuery();
        $result = $upgradeQuery->getListsByStatus(UpgradeQuery::WAIT_STATUS,$page,$pagesize);
        if($result){
            $lists = $result['lists'];
            $pages = pages($result['totalCount'],$page,$pagesize);
        }
    ...
    default:
        $upgradeQuery = new UpgradeQuery();
        $result = $upgradeQuery->getListsByStatus(UpgradeQuery::CHECKED_STATUS,$page,$pagesize);
        if($result){
            $lists = $result['lists'];
            $pages = pages($result['totalCount'],$page,$pagesize);
        }
