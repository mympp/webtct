...
class message {
	...
	function send($message) {
		...
		if (!$this->username&&!empty($message['username'])) {
			$this->username = '游客-'.$message['username'];
		}
	 ...
	 */
	function admin_read() {
		$itemids =$this->check_itemids('isread',$this->itemid,0);
		$result = $this->db->query("SELECT touser FROM {$this->pre}message WHERE itemid IN($itemids)");
		$read_users = array();
		while($r = $this->db->fetch_array($result)) {
			$read_users[] = $r['touser'];
		}
	 ...
	 * 管理员批量唯读站内信
	 */
	function admin_unread() {
		$itemids =$this->check_itemids('isread',$this->itemid,1);
		$result = $this->db->query("SELECT touser FROM {$this->pre}message WHERE itemid IN($itemids)");
		$read_users = array();
		while($r = $this->db->fetch_array($result)) {
			$read_users[] = $r['touser'];
		}		
		$this->db->query("UPDATE {$this->pre}message SET isread=0 WHERE itemid IN($itemids)");
		foreach ($read_users as $key => $user) {
	 ...
	 */
	function admin_send() {
		$itemids =$this->check_itemids('issend',$this->itemid,0);
		$this->db->query("UPDATE {$this->pre}message SET issend=1 WHERE itemid IN($itemids)");	
	}

	/**
	 * 管理员批量未转发站内信
	 */
	function admin_unsend() {
		$itemids =$this->check_itemids('issend',$this->itemid,1);
		$this->db->query("UPDATE {$this->pre}message SET issend=0 WHERE itemid IN($itemids)");
	}

	/**
	 * 检查ids
	 * @param string 检索的数据库字段
	 * @param string 数据的ID
	 * @param int    状态
	 * @return string 数据的ID字符串
	 */
	function check_itemids($col,$itemids,$status){
		$itemids = is_array($this->itemid) ? implode(',', $this->itemid) : intval($this->itemid);
		$sql = "SELECT itemid,$col FROM {$this->pre}message WHERE itemid IN ($itemids) ";
		$result = $this->db->query($sql);
		while ($r = $this->db->fetch_array($result)) {
			if($r[$col]!=$status){
				str_replace($r['itemid'], '', $itemids);
			}	
		}
