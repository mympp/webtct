...
switch($action) {
	case 'send':
		...
		if($submit) {
			...
			clear_upload($message['content']);
						if($forward){//线下结算后，让卖家点击进入管理中心
							$forward=str_replace('?action=order','',$forward);
							$message['content']=$message['content'].'<br><div class="messagebutton"><a href="'.$forward.'">进入订单管理中心</a></div>';
							}
			if($do->send($message)) {
	...
	case 'show':
		...
		if($status == 4 || $status == 3) {
			//修改：需要所有字符转换为小写后在比对
			if(strtolower($touser) != strtolower($_username)) message($L['message_msg_deny']);
			if(!$isread) {
				$do->read();
				--$_message;
