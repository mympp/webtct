<?php 
/*
date:201-9-1;who:chentao;where：行172；what:屏蔽qq必须为数字格式验证
*/
...
class member {
	...
	function is_member($member) {
		global $L, $AREA,$_groupid,$admin_user;
	...
	function add($member) {
		...
		$member['sound'] = 1;
		$member_fields = array('username','company','passport', 'password','payword','email','sound','gender','truename','mobile','msn','qq','ali','skype','department','career','groupid','regid','areaid','edittime','inviter','comefrom','passsalt', 'paysalt');
	...
	function edit($member)	{
		...
		$company_fields = array('company','type','areaid', 'catid','catids','business','mode','regyear','regunit','capital','size','address','postcode','telephone','fax','mail','homepage','sell','buy','introduce','thumb','keyword','linkurl','groupid','domain','icp','validated','validator','validtime','skin','m_skin','template','m_template','hits');
		...
		if($_POST['companys']) {
			$companys = file_vname($_POST['companys']);
			if($companys!=$member['company']){
            $r = $this->db->get_one("SELECT COUNT(*) AS num FROM {$this->table_member} WHERE company='$companys'");
			if($r['num']>0){message('您的网店名称与其他人重复，请返回修改！');}
			}
			$member_sql .= ",company='$companys'";
			$company_sql .=",company='$companys'";
		}
	...
	function login($login_username, $login_password, $login_cookietime = 0, $admin = false, $lock = true) {
		...
		if(!$user) {
			if($lock) $this->lock($login_lock, $LOCK, $DT_IP, $DT_TIME);
	...
	function rename($cusername, $nusername) {
		...
		foreach($MODULE as $m) {
			if($m['islink'] || $m['moduleid'] < 5) continue;
			$tables[] = in_array($m['module'], array('article', 'brand', 'buy', 'down', 'info', 'photo', 'sell', 'video', 'xinxi')) ? $m['module'].'_'.$m['moduleid'] : $m['module'];
		}
