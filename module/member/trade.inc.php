...
date:2015-12-2
who:chentao
what:修改线下付款的数据写入，买家确定收货时判断支付方式再进行资金和流水操作
where:见--2015/12/2/chentao--

date:2016/3/1
who:chentao
what:卖家关闭交易时，判断是否已线下交易完成，如果是不进行买家返还天成币
where：见--2016/3/1/chentao--
*/
...
if($action == 'update') {
	...
	switch($step) {
		...
		case 'send_goods'://卖家发货     //行232-241为20151015版升级包的验证处理
			...
			if($submit) {
				...
				$head_title = $L['trade_send_title'];      //--2015/12/2/chentao--start
		...
		case 'close'://关闭交易
			if($_username == $td['seller']) {
				if($td['status'] == 0) {
					...
					$money = $td['amount'] + $td['fee'];
					if($td['pay_type']==0){          //线上交易时执行返还金额，线下交易不执行      //--2016/3/1/chentao--start
						$db->query("UPDATE {$DT_PRE}member SET money=money+$money,locking=locking-$money WHERE username='$td[buyer]'");
					}else if($td['pay_type']==1){           //线下交易订单关闭
						$title=lang($L['trade_message_close_buyer_t'],array($td['itemid']));
						$url = $memberurl.'trade.php?action=order&itemid='.$td['itemid'];
						$content=lang($L['trade_message_close_buyer_c'],array($myurl, $_username, $timenow, $url));
						send_message($td['buyer'],$title,$content);	 //发送站内信提醒买家订单已关闭
					}                               //--2016/3/1/chentao--end
					$db->query("UPDATE {$table} SET status=9,updatetime=$DT_TIME WHERE order_mb_id=$order_mb_id");
					if($td['pay_type']==1){         //--2016/3/1/chentao--start
						message($L['trade_close_success'].',该交易已线下付款，请线下退款给买家!',$forward,120);
					}else{
						dmsg($L['trade_close_success'], $forward);
					}                               //--2016/3/1/chentao--end
				} else if($td['status'] == 8) {
	...
	$trades = array();
	#$result = $db->query("SELECT * FROM {$table} WHERE $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");
	$result = $db->query("SELECT order_mb_id, seller,buyer,status,addtime,group_concat(itemid),note,payment,updatetime,add_time,kefu_status FROM {$table} WHERE $condition group by order_mb_id ORDER BY itemid DESC LIMIT $offset,$pagesize");
	$amount = $fee = $money = 0;
	while($r = $db->fetch_array($result)) {
	...
	$trades = array();
	#$result = $db->query("SELECT * FROM {$table} WHERE $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");
	$result = $db->query("SELECT order_mb_id, seller,buyer,status,addtime,group_concat(itemid),note,payment,updatetime,add_time,payment,invoice,kefu_status FROM {$table} WHERE $condition group by order_mb_id ORDER BY itemid DESC LIMIT $offset,$pagesize");
	$amount = $fee = $money = 0;
	while($r = $db->fetch_array($result)) {
		if($r['status'] == 3) {
			$gone = $DT_TIME - $r['updatetime'];
