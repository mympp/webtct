...
if(in_array($action, array('', 'check', 'expire', 'reject', 'recycle'))) {
	$sfields = array('模糊', '标题', '产品品牌', '计量单位', '简介', '公司名', '联系人', '联系手机','联系电话', '联系地址', '电子邮件', '联系MSN', '联系QQ', '会员名', '编辑', 'IP', '参数名1', '参数名2', '参数名3', '参数值1', '参数值2', '参数值3', '文件路径', '内容模板');
	$dfields = array('keyword', 'title', 'brand', 'unit', 'introduce', 'company', 'truename', 'mobile','telephone', 'address', 'email', 'msn', 'qq','username', 'editor', 'ip', 'n1', 'n2', 'n3', 'v1', 'v2', 'v3', 'filepath', 'template');
...
switch($action) {
	...
	case 'taoxinxi':		
		if(is_array($itemid)) {
			...
			$result = $db->query("SELECT * FROM {$ftb} WHERE itemid in($v)");
			while($r = $db->fetch_array($result)) {
				$fid = $r['itemid'];
				unset($r['itemid']);
				$r['catid'] = $catid;
				...
				$sqlv = substr($sqlv, 1);
				//echo $sqlv;exit;
				$k = $db->get_one("SELECT itemid FROM {$ttb} WHERE content='{$content}'");
				if(!$k){
