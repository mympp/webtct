...
class sell {
	...
	function set($post) {
		...
		if($MOD['introduce_length']&&!$post['introduce']) $post['introduce'] = addslashes(get_intro($post['content'], $MOD['introduce_length']));
		if($this->itemid) {
			$new = $post['content'];
	...
	function update($itemid) {
		...
		if($member&&$member['groupid']>5) {
			foreach(array('groupid','vip','validated','company','areaid','truename','telephone','mobile','address','qq','msn','ali','skype') as $v) {
				if($item[$v] != $member[$v] && $item[$v] != ''){$update .= ",$v='".addslashes($item[$v])."'";} else {$update .= ",$v='".addslashes($member[$v])."'";}//TC添修 当表单为空时获取会员信息，不为空则按表单信息
			}
