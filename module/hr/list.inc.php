...
if($items) {
	if($_admin || $_username){
		...
		$result = $db->query("SELECT ".$MOD['fields']." FROM {$table} WHERE {$condition} GROUP BY company ORDER BY ".$MOD['order']." LIMIT {$offset},{$pagesize}", ($CFG['db_expires'] && $page == 1) ? 'CACHE' : '', $CFG['db_expires']);
	}
