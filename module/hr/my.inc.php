...
switch($action) {
	case 'add':
		...
		if($submit) {
			...
			if($itemid) {
				...
				$open = 3;
				if($_userid) {
					$r = $db->get_one("SELECT * FROM {$DT_PRE}hr_resume a,{$DT_PRE}hr_resume_data c WHERE a.itemid=c.itemid AND a.username='$_username' ORDER BY a.edittime DESC");
					if($r) {
						extract($r);
						list($byear, $bmonth, $bday) = explode('-', $birthday);
					} else {
	...
	case 'apply_delete':
		$itemid or message();
		$apply = $db->get_one("SELECT * FROM {$DT_PRE}hr_job_apply WHERE applyid='$itemid' AND apply_username='$_username'");
		if($apply) {
			if($apply['status']>0) $db->query("UPDATE {$DT_PRE}hr_job SET apply=apply-1 WHERE itemid='$apply[jobid]'");
			$db->query("DELETE FROM {$DT_PRE}hr_job_apply WHERE applyid='$itemid'");
		}
		dmsg($L['success_delete'], $forward);
	break;
	case 'apply':
		...
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}hr_job_apply a LEFT JOIN {$DT_PRE}hr_resume r ON a.resumeid=r.itemid LEFT JOIN {$DT_PRE}job j ON a.jobid=j.itemid WHERE a.apply_username='$_username' $condition");
		$pages = pages($r['num'], $page, $pagesize);		
		$lists = array();
		$result = $db->query("SELECT a.*,r.title AS resumetitle,j.title,j.linkurl FROM {$DT_PRE}hr_job_apply a LEFT JOIN {$DT_PRE}hr_resume r ON a.resumeid=r.itemid LEFT JOIN {$DT_PRE}hr_job j ON a.jobid=j.itemid WHERE a.apply_username='$_username' $condition ORDER BY a.applyid DESC LIMIT $offset,$pagesize");
...
if($_userid) {
	$nums = array();
	for($i = 1; $i < 4; $i++) {
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}hr_resume WHERE username='$_username' AND status=$i");
		$nums[$i] = $r['num'];
	}
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}hr_job_apply WHERE hr_apply_username ='$_username'");
	$nums['apply'] = $r['num'];
}
$head_title = $L['resume_manage'];
include template('my_hr_resume', 'member');
...
switch($action) {
	...
	case 'resume_show':
		$itemid or message();
		$resumeid or message();
		$db->query("UPDATE {$DT_PRE}hr_job_apply SET status=2 WHERE applyid='$itemid' AND job_username='$_username' AND status=1");
		dheader($MOD['linkurl'].rewrite('resume.php?itemid='.$resumeid));
	break;
	case 'resume_delete':
		$itemid or message();
		$apply = $db->get_one("SELECT * FROM {$DT_PRE}hr_job_apply WHERE applyid='$itemid' AND job_username='$_username' AND status>0");
		if($apply) {
			$db->query("UPDATE {$DT_PRE}hr_job_apply SET status=0 WHERE applyid='$itemid'");
			$db->query("UPDATE {$DT_PRE}hr_job SET apply=apply-1 WHERE itemid='$apply[jobid]'");
		}
		dmsg($L['success_delete'], $forward);
	break;
	case 'resume_invite':
		$itemid or message();
		$apply = $db->get_one("SELECT * FROM {$DT_PRE}hr_job_apply WHERE applyid='$itemid' AND job_username='$_username'");
		$apply or message($L['msg_not_apply']);
		$resume = $db->get_one("SELECT * FROM {$DT_PRE}hr_resume WHERE itemid='$apply[resumeid]'");
		...
		$job = $db->get_one("SELECT * FROM {$DT_PRE}hr_job WHERE itemid='$apply[jobid]' AND status=3");
		...
		$db->query("UPDATE {$DT_PRE}hr_job_apply SET status=3 WHERE applyid='$itemid' AND job_username='$_username' AND status>0");
	break;
	case 'resume':
		$condition = '';
		if($keyword) $condition .= " AND r.keyword LIKE '%$keyword%'";
		if($itemid) $condition .= " AND j.itemid=$itemid";
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}hr_job_apply a LEFT JOIN {$DT_PRE}hr_resume r ON a.resumeid=r.itemid LEFT JOIN {$DT_PRE}hr_job j ON a.jobid=j.itemid WHERE a.job_username='$_username' AND a.status>0 $condition");
		$pages = pages($r['num'], $page, $pagesize);		
		$lists = array();
		$result = $db->query("SELECT a.*,r.truename,r.catid,r.gender,r.education,r.school,r.areaid,r.age,r.experience,j.title,j.linkurl FROM {$DT_PRE}hr_job_apply a LEFT JOIN {$DT_PRE}hr_resume r ON a.resumeid=r.itemid LEFT JOIN {$DT_PRE}hr_job j ON a.jobid=j.itemid WHERE a.job_username='$_username' AND a.status>0 $condition ORDER BY a.applyid DESC LIMIT $offset,$pagesize");
		while($r = $db->fetch_array($result)) {
	...
	case 'talent_delete':
		$itemid or message();
		$db->query("DELETE FROM {$DT_PRE}hr_job_talent WHERE username='$_username' AND talentid=$itemid");
		dmsg($L['success_delete'], $forward);
	break;
	case 'talent':
		$condition = '';
		...
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}hr_job_talent t LEFT JOIN {$DT_PRE}hr_resume r ON t.resumeid=r.itemid WHERE t.username='$_username' $condition");
		$pages = pages($r['num'], $page, $pagesize);		
		$lists = array();
		$result = $db->query("SELECT * FROM {$DT_PRE}hr_job_talent t LEFT JOIN {$DT_PRE}hr_resume r ON t.resumeid=r.itemid WHERE t.username='$_username' $condition ORDER BY t.talentid DESC LIMIT $offset,$pagesize");
		while($r = $db->fetch_array($result)) {
...
if($_userid) {
	$nums = array();
	for($i = 1; $i < 5; $i++) {
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}hr_job WHERE username='$_username' AND status=$i");
		$nums[$i] = $r['num'];
	}
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}hr_job_talent WHERE username='$_username'");
	$nums['talent'] = $r['num'];
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}hr_job_apply WHERE job_username ='$_username' AND status>0");
	$nums['resume'] = $r['num'];
}
$head_title = lang($L['module_manage'], array($MOD['name']));
include template('my_hr_job', 'member');
