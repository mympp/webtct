...
//重新付款开始
if($action=='uppay'){
	$item = $db->get_one("SELECT * FROM {$DT_PRE}group_order WHERE itemid=$itemid and buyer='$_username'");
	$gitemid=$item['gid'];
	$amount = $item['number']*$item['price'];
	if($item['amount']<$amount)
	{
    if($amount > $_money) message($L['need_charge'], $MODULE[2][linkurl].'charge.php?action=pay&amount='.($amount-$_money));
	money_add($_username, -$amount);
	money_record($_username, -$amount, $L['in_site'], 'system', $L['group_order_credit'], 'ID('.$gitemid.')');
    $db->query("UPDATE {$DT_PRE}group_order SET amount=$amount WHERE itemid=$itemid");
	message($L['msg_buy_success'], $MODULE[2]['linkurl'].'group.php?action=order', 3);
	}
	else
	{message('已经支付完成，无需重复支付', $MODULE[2]['linkurl'].'group.php?action=order', 3);
	}
}
//正式开始
