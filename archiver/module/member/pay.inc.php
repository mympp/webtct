...
if(strpos($forward,'.php')&&substr($forward,-5)=='.html'){$forward=str_replace('.php?','-htm-',$forward);}
check_sign($_username.$mid.$itemid.$username.$fee.$fee_back.$currency.$forward.$title, $sign) or dalert($L['check_sign'].'提交数据:'.$_username.$mid.$itemid.$username.$fee.$fee_back.$currency.$forward.$title.'\n原数据:'.$sign.'\n后数据'.crypt_sign($_username.$mid.$itemid.$username.$fee.$fee_back.$currency.$forward.$title), $forward);
