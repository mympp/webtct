	...
	case 'add':
		if($submit) {
			...
			foreach($usernames as $username) {
				...
				$r = $db->get_one("SELECT username,money,locking FROM {$DT_PRE}member WHERE username='$username'");
				if(!$r) {
					$error .= '<br/>会员['.$username.']不存在';
					continue;
				}
				if($type==0&& $r['money'] < abs($amount)) {
					$error .= '<br/>会员['.$username.']余额不足，当前余额为:'.$r['money'];
					continue;
				}
				if($type==2&& $r['locking'] < abs($amount)) {
					$error .= '<br/>会员['.$username.']冻结余额不足，当前账号冻结金额有:'.$r['locking'];
					continue;
				}
				if($type==3&& $r['locking'] < abs($amount)) {
					$error .= '<br/>会员['.$username.']解冻余额不足，当前账号冻结金额有:'.$r['locking'];
					continue;
				}
				$reason or $reason = '现金';
				$note or $note = '手工';
				if($type<2){money_add($username, $amount);}
				if($type==2){money_add($username, -$amount);money_lock($username, $amount);$amount = -$amount;}
				if($type==3){money_unlock($username, $amount);}
