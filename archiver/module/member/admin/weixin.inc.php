...
if($openid) {
	...
	$menus = array (
		...
		array('待回复用户','?moduleid='.$moduleid.'&file='.$file.'&action=wait'),
		...
		array('被动回复', '?moduleid='.$moduleid.'&file='.$file.'&action=reply'),
...
switch($action) {
	...
	case 'wait':
		...
		$condition .= isset($totime) ? 'and t.addtime < '.(strtotime($totime)+(3600*24)).' ' : '';
		$condition .= isset($fromtime) ? 'and t.addtime > '.(strtotime($fromtime)+(3600*24)).' ' : '';
		$items = $db->get_one("select count(*) as num from $temporary_table as t where $condition");

		if($items){

			$pages = pages($items['num'], $page, $pagesize);
			$result = $db->query("select t.*,u.username,u.nickname,u.sex,u.city,u.province,u.country,u.language,u.headimgurl from $temporary_table as t join {$DT_PRE}weixin_user as u on t.openid = u.openid where $condition group by t.openid order by addtime desc LIMIT $offset,$pagesize");
			if($result){
				$lists = array();
				while($r=$db->fetch_array($result)){
					$r['adddate'] = timetodate($r['addtime'], 5);
			...
			if(empty($lists)){
				$l = $db->query("select * from $temporary_table as t where $condition");
				while($r = $db->fetch_array($l)){
					$db->query("update `tc_weixin_chat` set kefu_status = 1 where openid = '".$r['openid']."'");
				}
			}
			$TYPE = array(
				'text' => '文本消息',
			...
			include tpl('weixin_wait', $module);
		}
	break;
	case 'delete_openid':
		$open_id or msg('请选择记录');
		$open_ids = is_array($open_id) ? implode("','", $open_id) : $open_id;
		$open_ids = "'$open_ids'";
		$db->query("DELETE FROM {$DT_PRE}weixin_chat WHERE openid IN ($open_ids)");
		dmsg('删除成功', $forward);
	break;
	case 'wait_close':
		$open_id or msg('未选择用户');
		$open_ids = is_array($open_id) ? implode("','", $open_id) : $open_id;
		$open_ids = "'$open_ids'";
		$db->query("update {$DT_PRE}weixin_chat set kefu_status = 1 where openid in ($open_ids)");
		dmsg('关闭成功',$forward);
	break;
	case 'reply':		//自动回复管理
		require_once DT_ROOT.'/include/tcdb.class.php';
		$weixin_reply = new tcdb('weixin_reply');
		$type1_rules = $weixin_reply->where(['type'=>1])->all();
		$type0_rules = $weixin_reply->where(['type'=>0])->all();
		include tpl('weixin_reply',$module);
	break;
	case 'reply_show':
		if(empty($itemid)) msg('参数错误');
		require_once DT_ROOT.'/include/tcdb.class.php';
		$weixin_reply = new tcdb('weixin_reply');
		$rule = $weixin_reply->where(['itemid'=>$itemid])->one();
		include tpl('weixin_reply_show',$module);
	break;
	case 'reply_edit':
		require_once DT_ROOT.'/include/tcdb.class.php';
		$weixin_reply = new tcdb('weixin_reply');
		if(!empty($move) && $move == 'delete' && !empty($itemid)){
			$weixin_reply->where(['itemid'=>$itemid])->delete();
			dmsg('删除成功',"?moduleid=$moduleid&file=$file&action=reply");
		}
		
		if(!empty($post)){
			if($move == 'edit'){
				$weixin_reply->where(['itemid'=>$itemid])->edit($post);
				dmsg('修改成功',"?moduleid=$moduleid&file=$file&action=$action&move=$move&itemid=$itemid");
			}elseif($move == 'add'){
				if(empty($post['keyword'])) msg('请填写关键词');
				$post['type'] = 0;
				$weixin_reply->add($post);
				$insert_id = $weixin_reply->getInsertId();
				dmsg('添加成功',"?moduleid=$moduleid&file=$file&action=$action&move=edit&itemid=$insert_id");
			}else{
				msg('参数错误');
			}
		}else{
			$rule = $weixin_reply->where(['itemid'=>$itemid])->one();
			include tpl('weixin_reply_edit',$module);
		}	
	break;
	default:
		$sfields = array('按条件', '消息内容', '微信昵称', '会员名', '网站编辑');
