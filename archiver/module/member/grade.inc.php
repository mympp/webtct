...
if($MG['fee_mode']==0){
		$item = $db->get_one("SELECT * FROM {$DT_PRE}upgrade WHERE username='$_username' ORDER BY itemid DESC");
		$user = $item['username'] ? userinfo($item['username']) : array();
		$gsql = $msql = $csql = '';
		$gsql = "edittime=$DT_TIME,editor='$_username',status=3,note='免费组自动升级'";
				$msql = $csql = "groupid=$item[groupid],company='$item[company]'";
				$vip = $GROUP[$item['groupid']]['vip'];
				$csql .= ",vip=$vip,vipt=$vip";
		$db->query("UPDATE {$DT_PRE}upgrade SET $gsql WHERE itemid=$item[itemid]");
	 ...
	 message('恭喜！免费升级完成，点击进入完善你的相关资料！', $MODULE[2][linkurl].'edit.php?tab=2', 5);
}
else{
 message($L['grade_msg_success'], DT_PATH.'member', 5);
}
