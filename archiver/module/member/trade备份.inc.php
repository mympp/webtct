	...
	switch($step) {
		...
		case 'outline'://线下收款
			if($td['status'] > 1 || $td['seller'] != $_username) message($L['trade_msg_deny']);
				...
				$db->query("UPDATE {$table} SET status=2,updatetime=$DT_TIME,pay_type=1 WHERE itemid=$itemid");    //线下付款添加pay_type=1
					$touser = $td['buyer'];
					$title = lang($L['trade_message_t1'], array($itemid));
					$url = $memberurl.'trade.php?action=order&itemid='.$itemid;
					$content = lang($L['trade_message_c1'], array($myurl, $_username, $timenow, $url));
					...
					money_add($touser, $money);
						money_record($touser, $money, '线下付款', 'system', '线下汇款充值',$L['trade_order_id'].$itemid);
					money_add($touser, -$money);
						money_record($touser, -$money, '线下付款','system', '确认订单扣费', $L['trade_order_id'].$itemid);
					money_add($td['seller'], $money);
						money_record($td['seller'], $money, '线下付款', 'system', '收到订单汇款',$L['trade_order_id'].$itemid);
					money_add($td['seller'], -$money);
						money_record($td['seller'], -$money, '线下付款', 'system', '订单金额提现', $L['trade_order_id'].$itemid);
		...
		case 'receive_goods'://确认收货
			...
			//交易成功
			$money = $td['amount'] + $td['fee'];
			$mall_order_pay_type=$db->get_one("select pay_type from {$table} where itemid=$itemid");
		...
		//var_dump("update {$table} set payment='$thumb' where itemid=$itemid");
			$db->query("update {$table} set payment='$thumb' where itemid=$itemid");
		...
		case 'invoice':       //处理发票选项
			...
			$db->query("update {$table} set invoice='$invoice' where itemid=$itemid");
