...
$UA = strtoupper($_SERVER['HTTP_USER_AGENT']);//判断客户端--吕保成--2015-1-7
$UA = strpos($UA, 'WINDOWS NT');
if($UA){
	login();
}else{
	if(!$_username){dheader($EXT['mobile_url'].'login.php?forward='.DT_PATH.$_SERVER[REQUEST_URI]);}
}
...
if($itemid) {
	...
	if($action == 'buy' && $r['fee']) {
		...
		$c = $db->get_one("select * FROM {$db->pre}member_group WHERE groupname like '%vip%' ");
		if($r['fee'] and $_groupid!=$c['groupid'] ) dheader('?action=buy&itemid='.$itemid);
		$c = $db->get_one("SELECT skin,m_skin FROM {$DT_PRE}company WHERE userid=$_userid");
		$c['skin'] or $c['skin'] = 'default';
		$o = $db->get_one("SELECT itemid FROM {$DT_PRE}style WHERE skin='$c[skin]'");
		if($o) $db->query("UPDATE {$DT_PRE}style SET hits=hits-1 WHERE itemid=$o[itemid] AND hits>1");
		$db->query("UPDATE {$DT_PRE}style SET hits=hits+1 WHERE itemid=$itemid");
		if ($UA == false) 
			$db->query("UPDATE {$DT_PRE}company SET m_skin='$r[skin]' WHERE userid=$_userid");
		else
			$db->query("UPDATE {$DT_PRE}company SET template='$r[template]',skin='$r[skin]',styletime=0 WHERE userid=$_userid");
		dmsg($L['style_msg_use_success'], 'style.php?select=yes#m_'.$itemid);
	}
}else {
	if($action == 'view') {
		$c = $db->get_one("SELECT skin FROM {$DT_PRE}company WHERE userid=$_userid");
		...
		$dorder  = array('listorder asc,addtime desc', 'addtime DESC', 'addtime ASC', 'hits DESC', 'hits ASC');
		...
		if ($UA == false) $condition .= " AND m_skin!=''";
		$c = $db->get_one("SELECT skin,m_skin,linkurl,domain,styletime FROM {$DT_PRE}company WHERE userid=$_userid");
		if(!$c['skin']) {
			if($MG['styleid']) {
...
if($UA !== false && !DT_DEBUG) {
	include template('style', $module);
}else{
	include template('style', 'mobile');
}
?>
