...
if(in_array($action, array('', 'domain'))) {//V5.0 升级 V6.0 添加
...
switch($action) {
	...
	case 'level':
		...
		dmsg('级别设置成功', $forward);
	break;
	case 'domain'://V5.0 升级 V6.0添加
		$lists = $do->get_list($condition, $dorder[$order]);
		$menuid = 1;
		include tpl('index', $module);
	break;
	case 'download':			//企业信息导出
		$totime = empty($totime) ? time() : strtotime($totime)+(3600*24*30);
		$fromtime = empty($fromtime) ? ($totime-(3600*24*30)) : strtotime($fromtime);
		$sql_str = "select m.company,c.username,m.regtime as regtime,m.regip,c.telephone,c.mail,m.mobile,m.email,m.truename,c.address from {$db->pre}company as c join {$db->pre}member as m on c.userid = m.userid where m.regtime > $fromtime and m.regtime < $totime and c.homepage <> '' ";
		$result = $db->query($sql_str);
		$result_arr = array();
		while($r = $db->fetch_array($result)){
			$r['regtime'] = date('Y-m-d',$r['regtime']);
			$result_arr[] = $r;
		}
		include_once DT_ROOT.'/include/csv.class.php';
		$title = array('公司名','用户名','注册时间','注册ip','企业电话','企业邮箱','用户电话','用户邮箱','真实姓名','企业地址');
		$filename = 'company_'.date('Y-m-d',$fromtime).'_'.date('Y-m-d',$totime);
		$csv =new csv();
		$csv->output_csv($title,$result_arr,$filename);
	break;
	default:
		...
		$sorder  = array('结果排序方式', VIP.'指数降序', VIP.'指数升序', '先关后开网店', '先开后关网店', '注册资本降序', '注册资本升序', '服务开始降序', '服务开始升序', '服务结束降序', '服务结束升序','浏览人气降序','浏览人气升序');
		$dorder  = array('userid DESC', 'vip DESC', 'vip ASC', 'closeshop DESC,userid desc', 'closeshop ASC,userid desc', 'capital DESC', 'capital ASC', 'fromtime DESC', 'fromtime ASC', 'pnum DESC', 'pnum ASC', 'hits DESC', 'hits ASC');
		$svalid = array('认证', '已通过' , '未通过');
		$MS = cache_read('module-2.php');
		$modes = explode('|', '经营模式|'.$MS['com_mode']);
		$types = explode('|', '公司类型|'.$MS['com_type']);
		...
		$valid = isset($valid) ? intval($valid) : 0;
		$level = isset($level) ? intval($level) : 0;
		$nocatids = isset($nocatids) ? intval($nocatids) : 0;
		$uid = isset($uid) ? intval($uid) : '';
		$username = isset($username) ? trim($username) : '';
		isset($fromtime) or $fromtime = '';
		isset($totime) or $totime = '';
		...
		if($nocatids) $condition .= " AND catids=''";
		if($valid) $condition .= $valid == 1 ? " AND validated=1" : " AND validated=0";
		if($catid) $condition .= " AND catids LIKE '%,".$catid.",%'";
		if($areaid) $condition .= ($ARE['child']) ? " AND areaid IN (".$ARE['arrchildid'].")" : " AND areaid=$areaid";
		if($mode) $condition .= " AND mode LIKE '%$modes[$mode]%'";
		...
		if($mincapital) $condition .= " AND capital>$mincapital";
		if($maxcapital) $condition .= " AND capital<$maxcapital";
		if($thumb==1)  $condition .= " AND thumb<>''";
		if($thumb==2)  $condition .= " AND thumb=''";
		if($dzh)  $condition .= " AND business=''";
		if($closeshop)  $condition .= " AND closeshop=1";
		if($uid) $condition .= " AND userid=$uid";
		if($username) $condition .= " AND username='$username'";
		if($fromtime) $condition .= " AND $timetype>".(strtotime($fromtime.' 00:00:00'));
		if($totime) $condition .= " AND $timetype<".(strtotime($totime.' 23:59:59'));
		if($noResultOrder==1) $dorder[$order] = 'hits DESC';
