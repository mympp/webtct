...
$menus = array (
    ...
    array('不通过广告','javascript:Dwidget(\'?moduleid='.$moduleid.'&file='.$file.'&action=list&job=reject\', \'不通过广告\');'),
...
$menusad = array (
    ...
    array('不通过广告','?moduleid='.$moduleid.'&file='.$file.'&action=list&job=reject'),
);
...
switch($action) {
	...
	case 'list':
		...
		$status_name = array('1'=>'不通过','2'=>'待审核','3'=>'已通过');
