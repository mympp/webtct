...
$item = $db->get_one("SELECT * FROM {$DT_PRE}rc_resume WHERE itemid=$itemid AND status=3");
...
$item = $db->get_one("SELECT * FROM {$DT_PRE}rc_job_talent WHERE rc_resumeid=$itemid AND username='$_username'");
if($item) message($L['msg_talent_exist'], $linkurl);
$db->query("INSERT INTO {$DT_PRE}rc_job_talent (rc_resumeid,username,jointime) VALUES ('$itemid','$_username','$DT_TIME')");
