...
who:chentao
what:修改下载模板功能
where:见--/15/11/14/chentao--标签
relation:删除/moudle/rc_job/admin/download.inc.php ,修改/module/rc_job/admin/template/index.tpl.php,修改/module/rc_job/admin/download/resume_index.tpl.php

time:2016-1-27
who:chentao
what:修复招聘页面不显示招聘定义字段的问题，修复csv下载乱码问题，添加简历投递数统计
where:见--/16/1/27/chentao--标签
*/
...
if(in_array($action, array('add', 'edit'))) {
	$FD = cache_read('fields-hr_job.php');         //--/16/1/27/chentao--,fields-hr.php改为fields-hr_job.php
...
switch($action) {
	...
	break;
	case 'download':                //下载模板        //--/15/11/14/chentao--start
		$file_dir=DT_ROOT.'/file/hr_job/';
		if($download_file=='resume'){
			$file_name=$download_file.'.xls';
		}else{
			$file_name=$download_file.'.csv';
		}
		if(is_file($file_dir.$file_name)){
			ob_end_clean();                                      //--/16/1/27/chentao--
			header("Content-Type: application/force-download");  //--/16/1/27/chentao--
			header("Content-Type: text/csv");                    //--/16/1/27/chentao--
	...
	break;     //--/15/11/14/chentao--end
	default:
		$lists = $do->get_list('status=3'.$condition, $dorder[$order]);
		$menuid = 1;
		//--/16/1/27/chentao--start              //统计简历投递数
		$job_id_str='';
		foreach($lists as $k=>$v){
			$job_id_str.=$v['itemid'].',';
		}
		$count_apply_data=$db->query("select count(*) as c,resumeid from {$db->pre}hr_job_apply where status=1 and resumeid in (".substr($job_id_str,0,-1).") group by resumeid");
		$count_apply=array();                //统计结果数组
		while($v=mysql_fetch_array($count_apply_data,MYSQL_ASSOC )){
			$count_apply[$v['resumeid']]=$v['c'];
		}
		//--/16/1/27/chentao--end
		include tpl('index', $module);
	break;
