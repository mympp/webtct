...
class job {
	...
	function get_list($condition = 'status=3', $order = 'edittime DESC', $cache = '',$distinct=0) {
		global $MOD, $pages, $page, $pagesize, $offset, $CATEGORY, $items, $sum, $_admin ,$mid ,$company_nums;
		if($page > 1 && $sum) {
			$items = $sum;
		} else {
			if($_admin || $mid == 28){
				...
				$r = $this->db->get_one("SELECT COUNT(distinct company) AS num FROM {$this->table} WHERE $condition", $cache);
			}
			$items = $r['num'];
		}
		$cc = $this->db->get_one("SELECT COUNT(distinct company) AS num FROM {$this->table} WHERE $condition", $cache);
		$company_nums = $cc['num'];
		...
		if($_admin || $mid == 28){
			...
			$result = $this->db->query("SELECT * FROM {$this->table} WHERE $condition GROUP BY company ORDER BY $order LIMIT $offset,$pagesize", $cache);
		}
		if($distinct){$result = $this->db->query("SELECT * FROM {$this->table} WHERE $condition GROUP BY company ORDER BY itemid desc LIMIT $offset,$distinct", $cache);}
	...
	function news_list($condition = 'status=3', $order = 'edittime DESC', $cache = '',$distinct=0) {
		global $MOD, $pages, $page, $pagesize, $offset, $CATEGORY, $items, $sum, $_admin ,$mid ,$company_nums,$DT_PRE;
		if($page > 1 && $sum) {
			$items = $sum;
		} else {
				$r = $this->db->get_one("SELECT COUNT(*) AS num FROM ".$DT_PRE."article_21 WHERE $condition", $cache);
			$items = $r['num'];
		}
		$pages = defined('CATID') ? listpages(1, CATID, $items, $page, $pagesize, 10, $MOD['linkurl']) : pages($items, $page, $pagesize);
		$lists = array();
		$sql="SELECT * FROM ".$DT_PRE."article_21 WHERE $condition ORDER BY itemid desc LIMIT $offset,$distinct";
		$result = $this->db->query($sql, $cache);
		while($r = $this->db->fetch_array($result)) {
			$r['alt'] = $r['title'];
			$r['title'] = set_style($r['title'], $r['style']);
			$r['userurl'] = userurl($r['username']);
