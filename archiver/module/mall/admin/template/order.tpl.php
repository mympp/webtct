...
客服状态：
<select name="kefu_status">
<option value="" <?php if($kefu_status == '') echo 'selected=""'; ?>></option> 
<option value="0" <?php if($kefu_status === 0) echo 'selected=""'; ?> >未介入</option>
<option value="1" <?php if($kefu_status == 1) echo 'selected=""'; ?> >处理中</option>
<option value="2" <?php if($kefu_status == 2) echo 'selected=""'; ?> >已结束</option>
</select>
</td>
</tr>
<tr>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
逾期确认订单:<input type="checkbox" name="over_check" value="1" <?php if($over_check == '1') echo 'checked=""'; ?> />&nbsp;&nbsp;&nbsp;&nbsp;
逾期完成订单:<input type="checkbox" name="over_finish" value="1" <?php if($over_finish == '1') echo 'checked=""'; ?> />
</td>
</tr>
<?php } ?>
</table>
...
<?php if($over_check == '1' || $over_finish == '1'){ ?>
<td class="px11"><b style="color:red;"><?php echo $v['endtime']; ?></b></td>
<?php } ?>
<?php if($kefu_status){ ?>
<?php $kefu_status_arr = [1=>'处理中',2=>'已完成']; ?>
<td class="px11"><?php echo $kefu_status_arr[$kefu_status]; ?></td>
<?php } ?>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
订单状态：
<?php $edit_status_select = dselect($dstatus, 'edit[status]', '状态', $status, 'style="width:200px;"', 1, '', 1); ?>
<?php echo $edit_status_select;?>&nbsp;
客服状态：
<select name="edit[kefu_status]">
	<option value="0" selected="">未介入</option>
	<option value="1">处理中</option>
	<option value="2">已完成</option>
</select>&nbsp;
客服备注：
<input type="text" name="edit[kefu_remark]" />&nbsp;
<input type="submit" value=" 批量修改 " class="btn" onclick="if(confirm('确定要修改选中记录吗？')){this.form.action='?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=edit'}else{return false;}" />&nbsp;&nbsp;
</div>
</form>
