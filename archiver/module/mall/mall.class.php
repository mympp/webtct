...
class mall {
    ...
    function mall($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','mycatid','areaid','level','title','style','fee','introduce','brand','price','bprice','step','amount','unit','thumb','thumb1','thumb2','tag','status','hits','username','editor','addtime','adddate','edittime','editdate','ip','template','linkurl','filepath','elite','note','company','truename','telephone','mobile','address','email','msn','qq','ali','skype','n1','n2','n3','v1','v2','v3','express_1','express_name_1','fee_start_1','fee_step_1','express_2','express_name_2','fee_start_2','fee_step_2','express_3','express_name_3','fee_start_3','fee_step_3','stype','photocatid','bpriceovert','cod','order_mb_id','manufacturer','batchnum');
    }
	...
	function set($post) {
		...
		$post['price'] = ($post['step']['p1']==''||$post['step']['p1']==0)?$post['price']:$post['step']['p1'];
		...
		$post['bpriceovert'] = $post['bpriceovert'] ? 1 : 0;
	...
	function get_one() {
		...
		$items = $this->db->get_one("SELECT * FROM {$this->table} WHERE itemid=".$this->itemid);
		if($items){
			$items = $this->db->get_one("SELECT * FROM {$content_table} WHERE itemid=".$this->itemid);
			if(!$items){ 
				$this->db->query("INSERT INTO {$content_table} (itemid,content) VALUES ($this->itemid,'Ξ')");
			}
		}
	...
	function update($itemid) {
		...
		if($member&&$member['groupid']>5) $update .= update_user($member, $item);
	...
	function reject($itemid,$note = '') {
		global $_username, $DT_TIME;
		if(is_array($itemid)) {
			foreach($itemid as $v) { $this->reject($v,$note); }
		} else {
			$this->db->query("UPDATE {$this->table} SET status=1,editor='$_username',note = '$note' WHERE itemid=$itemid");
			return true;
		}
	}
