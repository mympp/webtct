...
<tr>
<td class="tl"><span class="f_red">*</span>专题分区</td>
<td>
<script type="text/javascript" src="<?php echo DT_PATH;?>file/script/jquery.js"></script>
<script type="text/javascript" src="<?php echo DT_PATH;?>file/script/lhgdialog.js?skin=idialog"></script>
<script type="text/javascript" src="<?php echo DT_PATH;?>file/script/jsfunction.js"></script>
<input name="post_fields[specialid]" type="text" size="30" value="<?php echo $specialid;?>" id="specialid" style="display:none"/>
<input type="text" size="60" value="<?php echo $specialtitle;?>" name="post[tag]" id="specialtitle"  onfocus="msgbox('挑选专题','<?php echo $MODULE[11][linkurl]?>select.php?kw='+encodeURI(Dd('specialtitle').value))" />
<?php if($specialid){?>
	<script type="text/javascript">
	<?php $r = $db->get_one("SELECT * FROM ".$DT_PRE."special WHERE itemid=$specialid");?>
	Dd('specialtitle').value="<?php echo $r[title];?>";
	</script>
<?php }?>
<input type="button" class="submit" onclick="msgbox('挑选专题','<?php echo $MODULE[11][linkurl]?>select.php?kw='+encodeURI(Dd('specialtitle').value))" value="去查找" class="submit"/>选中某个专区，您的信息会归在该专题页面！！
<script type="text/javascript">
