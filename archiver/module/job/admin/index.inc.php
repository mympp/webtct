...
if(in_array($action, array('', 'check', 'expire', 'reject', 'recycle'))) {
	...
	$MARRIAGE[0] = '工作形式';
	$EDUCATION[0] = '学历要求';
	$sfields = array('模糊', '职位名称', '简介', '发布单位', '公司名', '联系人', '联系电话', '联系地址', '电子邮件', '联系MSN', '联系QQ', '会员名', 'IP');
	$dfields = array('keyword', 'title', 'introduce', 'department', 'company', 'truename', 'telephone', 'address', 'email', 'msn', 'qq','username', 'ip');
	$sorder  = array('结果排序方式', '更新时间降序', '更新时间升序', '添加时间降序', '添加时间升序', '浏览次数降序', '浏览次数升序', '报名人数高到低', '报名人数低到高', '最低待遇降序', '最低待遇升序', '最高待遇降序', '最高待遇升序', '选定次数高到低', '选定次数低到高', '信息ID降序', '信息ID升序');
	$dorder  = array($MOD['order'], 'edittime DESC', 'edittime ASC', 'addtime DESC', 'addtime ASC', 'hits DESC', 'hits ASC', 'apply DESC', 'apply ASC', 'minsalary DESC', 'minsalary ASC', 'maxalary DESC', 'maxsalary ASC', 'total DESC', 'total ASC', 'itemid DESC', 'itemid ASC');
	...
	if($step) $condition .= " AND step=$step";
	if($apply<>'') $condition .= " AND apply".$dengyu.$apply;
