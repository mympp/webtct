...
if(in_array($action, array('', 'check', 'expire', 'reject', 'recycle'))) {
	...
	$sorder  = array('结果排序方式', '更新时间降序', '更新时间升序', '添加时间降序', '添加时间升序', '浏览次数降序', '浏览次数升序', '最低待遇降序', '最低待遇升序', '最高待遇降序', '最高待遇升序', '选定次数高到低', '选定次数低到高', '信息ID降序', '信息ID升序');
	$dorder  = array($MOD['order'], 'edittime DESC', 'edittime ASC', 'addtime DESC', 'addtime ASC', 'hits DESC', 'hits ASC', 'minsalary DESC', 'minsalary ASC', 'maxalary DESC', 'maxsalary ASC', 'talent DESC', 'talent ASC', 'itemid DESC', 'itemid ASC');
	...
	if($level0) $condition .= " AND level=0";
	...
	if($talent<>'') $condition .= " AND talent".$dengyu.$talent;
...
switch($action) {
	...
	default:
		...
		//已通过简历数目为$items
		//已通过和未通过简历数目
		$resume_count = $db->get_one("select count(*) as num from {$DT_PRE}resume where status in (2,3) $condition");
		//修改显示统计数目
