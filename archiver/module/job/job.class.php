...
class job {
	...
	function get_apply_list($condition) {
		global $MOD, $pages, $page, $pagesize, $offset, $CATEGORY, $items, $sum,$DT_PRE;
		if($page > 1 && $sum) {
			$items = $sum;
		} else {
			$r = $this->db->get_one("SELECT COUNT(*) AS num FROM {$this->table}_apply a WHERE  a.applyid>0 ".$condition, $cache);
			$items = $r['num'];
		}
		$pages = defined('CATID') ? listpages(1, CATID, $items, $page, $pagesize, 10, $MOD['linkurl']) : pages($items, $page, $pagesize);
		$lists = array();
		$result = $this->db->query("SELECT a.*,r.truename,r.title as title1,j.title,j.linkurl,j.apply FROM  {$DT_PRE}job_apply a LEFT JOIN  {$DT_PRE}resume r ON a.resumeid=r.itemid LEFT JOIN  {$DT_PRE}job j ON a.jobid=j.itemid WHERE a.applyid>0 ".$condition." ORDER BY a.applyid DESC LIMIT $offset,$pagesize", $cache);
		while($r = $this->db->fetch_array($result)) {
			$lists[] = $r;
		}
		return $lists;
