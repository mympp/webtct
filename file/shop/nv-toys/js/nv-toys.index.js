...
function getRandom(x,y){
    ...
    if( r==LastNum ) getRandom(x,y);//再次调用自身
    LastNum=r;//记录值
    return r;
}

function getVal(){
    var city = new Array("上海市","北京市","广州市","深圳市","天津市","重庆市","苏州市","武汉市","成都市","杭州市","南京市","青岛市","长沙市","无锡市","佛山市","宁波市","大连市","郑州市","沈阳市","烟台市","济南市","东莞市","泉州市","南通市","唐山市","西安市","哈尔滨市","福州市","长春市","石家庄市","合肥市","潍坊市","徐州市","常州市","温州市","绍兴市","鄂尔多斯市","大庆市","济宁市","淄博市","盐城市","扬州市","昆明市","南昌市","临沂市","东营市","包头市","台州市","泰州市","嘉兴市","洛阳市","厦门市","镇江市","金华市","南宁市","沧州市","襄阳市","宜昌市","邯郸市","泰安市","榆林市","惠州市","呼和浩特市","威海市","中山市","保定市","吉林市","鞍山市","南阳市","德州市","岳阳市","太原市","聊城市","乌鲁木齐市","常德市","漳州市","贵阳市","淮安市","衡阳市","茂名市","滨州市","芜湖市","湛江市","菏泽市","柳州市","株洲市","许昌市","江门市","咸阳市","廊坊市","枣庄市","周口市","连云港市","湖州市","宿迁市","新乡市","通辽市","遵义市","郴州市","兰州市")
    ...
    var sex =  new Array("先生", "女士");
    var telPostfix = new Array("134","135","136","137","138","139","150","151","152","157","158","159","187","188","130","131","132","155","156","185","186","133","153","180","189");
    var type = new Array("购买", "加盟");
    var product = new Array("糖心多功能","嘲笑鸟","玛塔","天使之吻","足球","异形","天琴座","海螺","飞机杯","刀锋战士","步行者","上帝之手","沙发","猫头","萝莎","萝拉","蝴蝶","耳机","约克","约克","纽曼","纽伦","曼波","手指扣","美容笔","AV震动"); 
    // 随机获取地区
    var c_i = getRandom(0,99);
    var cityVal = city[c_i];

    // 随机获取姓式
    var n_i = getRandom(0,99);
    var familyNameVal = familyNames[n_i];

    // 随机获取称呼
    var s_i = getRandom(0,1);
    var sexVal = sex[s_i];

    // 随机获取手机号码
    var t_i = getRandom(0,20);
    var telSuffix = "";
    for (var j = 0; j < 4; j++) {
        telSuffix = telSuffix + Math.floor(Math.random() * 10);
    }
    var telVal = telPostfix[t_i] + "****" + telSuffix;

    // 随机获取类型
    var y_i = getRandom(0,1);
    var typeVal = type[y_i];

    // 随机获取产品
    var p_i = getRandom(0,26);
    var productVal = product[p_i];

    var itemVal = "来自"+cityVal+"的"+familyNameVal+sexVal+"("+telVal+")已成功";
    // 如果是产品类型读取产品值
    if( y_i == 0 ){
        return itemVal+typeVal+productVal;
    }else{
        return itemVal+typeVal;
    }
}
...
function htmlData(){
    var html = "";
    for (var i = 0; i < 20; i++) {
        html = html + "<li>"+getVal()+"</li>"
    }
    $(".already-list").html(html);
    // 已加盟伙伴
    jQuery(".already").slide({
        mainCell:"ul",
        autoPlay:true,
        effect:"topMarquee",
        vis:8,
