$(function () {
   ...
   $(document).on("click",".edit-shop-btn",function(){
        ...
        $(this).hide();
        $wrap.find(".finish-edit-shop-btn").show();
        $wrap.find(".shop-remove-btn").show();
        $wrap.find(".shop-info-text").addClass("shop-info-text-edit");
    });

    //完成商品编辑状态
    $(document).on("click",".finish-edit-shop-btn",function(){
        var $wrap = $(this).parent().parent().parent();
        $(this).hide();
        $wrap.find(".edit-shop-btn").show();
        $wrap.find(".shop-remove-btn").hide();
        $wrap.find(".shop-info-text").removeClass("shop-info-text-edit");
    });

    $(document).on("click",".shop-remove-btn",function(){
        var $this = $(this);
        var $wrapHide = false;
        if($this.parent().parent().find(".shop-info").length == 1){
            $wrapHide = true;
        }
