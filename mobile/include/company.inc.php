...
if($userid || $username) {
	...
	$HURL1 = homepageurl($moduleid, $username);
	...
	switch($action) {
		case 'introduce':
			...
			if($itemid) {
				...
				// $back_link = $HURL1.'&action='.$action;
				$back_link = homepageurl($moduleid, $username, 0, $action);
		...
		break;
		case 'honor':
			isset($_MENU[$action]) or dheader($HURL1);
			$table = $DT_PRE.'honor';
			$head_name = $_MENU[$action];
			$head_title = $head_name.$DT['seo_delimiter'].$head_title;
			if($itemid) {
				...
				$date = timetodate($addtime, 3);
				// $back_link = $HURL.'&action='.$action;
				$back_link = mobileurl($moduleid, $username, 0, $action);
				$head_title = $title.$DT['seo_delimiter'].$head_title;
			} else {
				$condition = "username='$username' AND status=3";
				$r = $db->get_one("SELECT COUNT(*) AS num FROM {$table} WHERE $condition", 'CACHE');
		...
		case 'type':
			...
			$back_link = mobileurl($moduleid, $username, 0, $item);
			include template('homepage-'.$action, 'mobile');
		break;
		case 'mall':
			$moduleid = 16;
		...
		case 'message':
			if($submit) {
				if(!$fromuser || !$tel || !$content) wap_msg('留言信息请填写完整',mobileurl($moduleid, $username, 0, $action));
				...
				if($do->send($message)) {
					mobile_msg($L['message_success'],mobileurl($moduleid, $username, 0, $action));
				} else {
		...
		default:
			...
			$M_MENU = array('introduce','news','credit','contact');
			$M = array();
			foreach($MENU as $v) {
				if(in_array($v['file'], array('introduce', 'news', 'credit', 'contact'))) continue;
				$M[] = $v;
			}			
			$tpl = $db->get_one("SELECT m_template FROM {$DT_PRE}company WHERE userid=$userid");			
			$mulu = $preview ? $preview : $tpl['m_template'];			
			if ($preview) {
				include template($preview, 'mobile/homepage');
			}else{
				if ($COM['vip']) {								
					if (!empty($tpl['m_template'])) {					
						include template($tpl['m_template'], 'mobile/homepage');
					}else{
						$mulu = 'vip';
						include template('vip', 'mobile/homepage');
					}
				}else{
					include template('default', 'mobile/homepage');
				}
			}
		break;
	}
	if(in_array($action, array('mall', 'sell', 'buy', 'job', 'photo', 'info', 'brand', 'video'))) {
		...
		while($r = $db->fetch_array($result)) {
			$r['linkurl'] = mobileurl($moduleid, $r['catid'], $r['itemid']);
	...
	if($items) {
		...
		while($r = $db->fetch_array($result)) {
			...
			$r['linkurl'] = companyurl($moduleid, $r['username']);
			$lists[] = $r;
		}
	...
	if($kw) {
		...
		if($CAT['parentid']) $back_link = mobileurl($moduleid, 0, $CAT['parentid']);
	} else {
		$head_name = $MOD['name'];
	}
