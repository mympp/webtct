...
if($submit) {
	...
	if($post) {
		...
		foreach($post as $k=>$v) {
			...
			if($t && $t['status'] == 3 && $t['username'] != $_username && $t['amount'] > 0) {
				...
				send_message($_username, '站内交易提醒', '您于'.timetodate($DT_TIME, 3).'订购了'.$goods.',订单号为:'.$itemid.'订单金额为:'.$amount);
	...
	$do->set($cart);
	message($L['msg_buy_success'], $MODULE[1]['linkurl'].'wap/index.php?moduleid=2&action=message');
} else {
	$tags = $_tags = array();
	$receive = '';
	if(is_array($itemid)) {
	...
	include template('mall_buy','touch');
}
?>