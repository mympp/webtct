...
if($mid == 4) {
	...
	$table = get_table($mid);	
	if ($mid == 29) $table ='tc_news';
	$item = $db->get_one("SELECT title,linkurl,username,status,thumb FROM $table WHERE itemid=$itemid");
	$item or exit;
...
switch($action) {
	...
	case 'count':
		if($EXT['comment_api'] == 'changyan') {
			...
			$condition = "item_mid=$mid AND item_id=$itemid AND status in (2,3)";
