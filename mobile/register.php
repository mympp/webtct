...
$verify_type = '';
$MOD['checkuser'] = 0;//关闭移动注册验证 2015/12/4
$need_check = 0;
			...
			if($need_check) {
				...
				if($user) {
					...
					if(get_cookie('weixin_openid')) {
						...
						mobile_msg('恭喜！注册并绑定微信会员成功！', 'weixin.php?action=bind');
					}
					if(get_cookie('qq_openid')) {
						$openid = decrypt(get_cookie('qq_openid'), DT_KEY.'QQID');
						$db->query("UPDATE {$DT_PRE}oauth SET username='$username' WHERE openid='$openid'");
						mobile_msg('恭喜！注册并绑定QQ会员成功！', $forward ? $forward : 'my.php');
					}
					unset($_SESSION['m_name']);
