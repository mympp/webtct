...
<!-----------TC 邮局配置------------->
<tr>
<td class="tl">TC 邮局配置</td>
<td>
设置每个发件箱每天的邮件发送次数：<input name="setting[sendMailCount]" type="text" value="<?php echo $sendMailCount;?>" size="5"/><?php tips('此值为每个发件箱每天的邮件发送次数，完后跳至下个发件箱继续发送剩余邮件，$DT[sendMailCount]。');?><br/>
<input type="button" value="设置TC邮局SMTP服务器" onclick="javascript:Dwidget('?file=setting&action=tc_mail','配置TC邮局的邮件帐号密码、SMTP服务器、SSL、端口');"/><?php tips('配置TC邮局的邮件帐号密码、SMTP服务器、SSL、端口 ');?>
</td>
</tr>
<!-----------TC 邮局配置------------->
</tbody>
...
<font color="red">因使用TC邮局，此选项需要开启！</font>
</td>
</tr>
<tr> 
