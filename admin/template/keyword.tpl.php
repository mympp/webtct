...
<td><?php if(isset($keyword_cats[$v['parent_catid']])) echo $keyword_cats[$v['parent_catid']]; ?></td>
...
<td><input type="button" class="btn" value="添加" onclick="Dwidget('?file=keyword&action=link_list&key_id=<?php echo $v['itemid'];?>', '友情链接')"/></td>
...
&nbsp;&nbsp;<input type="submit" name="submit" value="删 除" onclick="if(_del && !confirm('提示:您选择删除'+_del+'个关键词？确定要删除吗？')){ return false;}else{Dd('move').value='delete';return true;}" class="btn"/>
&nbsp;&nbsp;<input type="submit" name="submit" value="<?php if($status==3){echo '待 审';}else{echo '启动';} ?>" onclick="if(_del && !confirm('提示：你选择将'+_del+'个关键词从<?php if($status==3){ echo '启动';}else{echo '待审';} ?>改为<?php if($status==3){echo '待审';}else{echo '启动';} ?>? 确定要修改？')){return false;}else{Dd('move').value='change_status';return true;}" class="btn" />
