...
switch($action) {
	case 'add':
		if($submit) {
			...
			$db->query("INSERT INTO {$DT_PRE}fetch (sitename,domain,encode,title,content,editor,edittime,thumb,contentreplace,fieldvalue) VALUES ('$sitename','$domain','$encode','$title','$content','$_username','$DT_TIME','$thumb','$contentreplace','$fieldvalue')");
			dmsg('添加成功', $forward);
		} else {
	...
	case 'edit':
		...
		if($submit) {
			if(!$domain) msg('请输入采编域名');
			if(strpos($content, '[content]') === false) msg('请输入内容规则');
			$db->query("UPDATE {$DT_PRE}fetch SET sitename='$sitename',domain='$domain',encode='$encode',title='$title',content='$content',editor='$_username',edittime='$DT_TIME',thumb='$thumb',contentreplace='$contentreplace',fieldvalue='$fieldvalue' WHERE itemid=$itemid");
			dmsg('修改成功', $forward);
		} else {
