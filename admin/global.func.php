...
// 原生V5.0、V6.0无此代码块 开始
         if($r['contentreplace']){
			...
			$k= str_replace(array("\r\n", "\r", "\n"), "", $k); 
			$kk=explode("|||", $k);
			$fetch['content']=str_replace($kk[0],$kk[1], $fetch['content']);
			}
		 }
	 if($r['thumb']){
			list($f, $t) = explode('[thumb]', $r['thumb']);
			if($f && $t) {
				$s = strpos($content, $f);
				if($s !== false) {
					$e = strpos($content, $t, $s);
					if($e !== false && $e > $s) {
						$fetch['thumb'] = substr($content, $s + strlen($f), $e - $s - strlen($f));
					}
				}
			}
		 }
...
function edition($k = -1) {
	...
	if(is_file($F)) {
		...
		if($E[0] == $D || $local || strpos($_SERVER['HTTP_HOST'], '.'.$E[0]) !== false) {
			...
			dheader(decrypt('DTUEdVYkAyNYN1UtAy1WKgJ0Dn4KJAdlDT1RIFIgCGZRblA7UXkEN1ljAjg=', 'DT').'/authorization.php?pirate=true&domain='.$D);
		}
	} else {
		$E[] = $D;
		$E[] = base64_decode('JiMyMDAxMDsmIzIwMTU0OyYjMjkyNTY7');
	}
	return $k >= 0 ? $E[$k] : $E;
}
*/
function admin_log($force = 0) {
	...
	if($DT['admin_log'] == 2 || ($DT['admin_log'] == 1 && ($file == 'setting' || in_array($action, array('delete', 'edit', 'move', 'clear', 'add'))))) {
		...
		$md5string = md5($DT_QST);//V5.0 升级 V6.0 添加
...
function admin_notice() {  // V5.0 升级 V6.0 添加函数
...
function list_dir($dir) { // V5.0 升级 V6.0 修改
/*
*V5.0 代码块 返回 $dirs
*修改后返回 $list
	$dirs = array();
