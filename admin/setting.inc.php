...
switch($action) {
	...
	case 'tc_mail'://TC添修代码块
		...
		#		/admin/setting.inc.php -> 40行到120行 $action == 'tc_mail'
		...
		if($post['act'] == 'add'){
			$fp=fopen(DT_ROOT."/file/tc_email/emailSmtps.php","a");
			fwrite($fp,"#".$post["user"]."|".$post["pass"]."|".$post["smtp"]."|".$post["ssl"]."|".$post["dk"]."|".time()."\r\n"); 
			fclose($fp);
			dmsg('添加成功', '?file=setting&action=tc_mail');
		}else if($post['act'] == 'addTest'){
			$addTest=fopen(DT_ROOT."/file/tc_email/testEmail.txt","w");
			fwrite($addTest,$post["csmail"]);
			fclose($addTest);
			...
			foreach($lines as $key=>$value){
				$line=explode("|",$value);
				if (trim($line[5])==$time){ //遍历数组，定位符合条件的key，并修改该行
					$lines[$key]=$m1."|".$m2."|".$m3."|".$m4."|".$m5."|".$m6."\r\n";
					//break;
				}
			}
			unlink(DT_ROOT."/file/tc_email/emailSmtps.php");
			$fp=fopen(DT_ROOT."/file/tc_email/emailSmtps.php","w");
			$lines[0] = "<?php defined('IN_DESTOON') or exit('Access Denied');\r\n";
			foreach ($lines as $value) {
				fwrite($fp,$value); //写入修改后的新数组
			}
			fclose($fp);
			dmsg("修改成功", '?file=setting&action=tc_mail');
		}else if($post['act'] == 'delete'){
			$time=trim($post["time"]);
			foreach($lines as $key=>$value){
				$line=explode("|",$value);
				if (trim($line[5])==$time) { //遍历数组，定位符合条件的key，并删除改行
					unset($lines[$key]);
					break;
				}
			}
			...
			fclose($fp);
			dmsg("成功删除", '?file=setting&action=tc_mail');
		}else if($post['act'] == 'sendTest'){
			#function send_test_tc_mail 写在include/tece.func.php里
			send_test_tc_mail($post['tomail'], $post['mailtitle'], $post['mailcontent'], $post['user'], $post['emailpass'], $post['smtp'], $post['ssl'], $post['dk']);
			if(substr(trim($_SESSION['JG']),-3) == "Bye"){
					dmsg("发送测试邮件成功，请到目标邮件查看！", '?file=setting&action=tc_mail');
				}else{
					if(substr(trim($_SESSION['JG']),-6) == "first!"){
						dmsg("测试失败，该邮箱的SMTP服务没有开启！", '?file=setting&action=tc_mail');
					}else{
						dmsg("测试失败，邮箱或者密码或者SMTP有误！", '?file=setting&action=tc_mail');
					}
			}
		}//$post['act'] 判断结束
		include tpl('tc_mail', $module);
	break;
	case 'static': //V5.0 升级 V6.0 添加
		if($itemid) {
	...
	default:
		...
		if($submit) {
			foreach($setting as $k=>$v) {
				...
				seo_check($v) or msg('SEOф┐бцБпхМЕхРлщЭЮц│ХхнЧчмж');
			}
