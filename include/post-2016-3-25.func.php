...
function deditor($moduleid = 1, $textareaid = 'content', $toolbarset = 'Default', $width = 500, $height = 400) {
	global $DT, $MODULE, $_userid,$_groupid,$editortype;
	if(!$_REQUEST['editortype']&&!$editortype){
		$editor = '';
		...
		if($_groupid == 1){
			$editor .='<br/><input type="checkbox" onclick="if(this.checked==true){document.location.href=document.location.href+\'&editortype=1\';}">任何字符模式';
		}
	}else{
		$editor .='<script>Dd(\''.$textareaid.'\').style.display=\'block\';;Dd(\''.$textareaid.'\').style.width=\'700px\';Dd(\''.$textareaid.'\').style.height=\'400px\';</script>';
	}
	return $editor;
}
