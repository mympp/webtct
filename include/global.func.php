...
function template($template = 'index', $dir = '') {//V5.0 升级 V6.0 修改
	global $CFG;
	check_name($template) or exit('BAD TPL NAME');
	/*
	*V6.0升级添加，规范模板调用路径，除去原因：不可以使用二级文件路径
	*例如：template\tc\job\index.htm -> {template 'service_area', 'tecenet/html'}，如果添加后调用只能是{template 'service_area', 'tecenet'}
...
function get_table($moduleid, $data = 0) {
	...
	if($data) {
		if($module == 'hr'){
			return $DT_PRE.'hr_job_data';//TC添修，因为hr的module是hr,而数据库表示hr_job_data,在生产sitemap是报错
		}else{
			return in_array($module, $C) ? $DT_PRE.$module.'_data_'.$moduleid : $DT_PRE.$module.'_data';
		}
	}else{
		if($module == 'hr'){
			return $DT_PRE.'hr_job';//TC添修,因为hr的module是hr,而数据库表示hr_job
		}else{
...
function send_message($touser, $title, $content, $typeid = 4, $fromuser = '',$fromurl='') {
	//TC添修文件，$fromurl 新增字段
	...
	if(check_name($touser) && $title && $content) {
		...
		if($r) {
			...
			$db->query("INSERT INTO {$db->pre}message (title,typeid,touser,fromuser,content,addtime,ip,status,fromurl) VALUES ('$title', $typeid, '$touser','$fromuser','$content','$DT_TIME','$DT_IP',3,'$fromurl')");			
			$db->query("UPDATE {$db->pre}member SET message=message+1 WHERE username='$touser'");
			if($fromuser) {
				$db->query("INSERT INTO {$db->pre}message (title,typeid,content,fromuser,touser,addtime,ip,status,fromurl) VALUES ('$title','$typeid','$content','$fromuser','$touser','$DT_TIME','$DT_IP',2,'$fromurl')");
			}
			return true;
		}
	}
...
function dt_send_mail($mail_to, $mail_subject, $mail_body, $mail_from = '', $mail_sign = true) {
	global $DT;
...
function send_weixin($touser, $word) {//V5.0 升级 V6.0 添加
...
function get_type($item, $cache = 0) {//TC添修文件，添加parentid字段
	$types = array();
	if($cache) {
		$types = cache_read('type-'.$item.'.php');
	} else {
		global $db;
		$result = $db->query("SELECT * FROM {$db->pre}type WHERE item='$item' and parentid=0 ORDER BY listorder ASC,typeid DESC ");
		while($r = $db->fetch_array($result)) {
			$types[$r['typeid']] = $r;

				$dresult = $db->query("SELECT * FROM {$db->pre}type WHERE item='$item' and parentid=".$r['typeid']." ORDER BY listorder ASC,typeid DESC ");
				while($dr = $db->fetch_array($dresult)) {
					$types[$dr['typeid']] = $dr;
				}
		}
	}
...
time:2015-12-10
who:xiaolv
修改cat_pos函数，增加对手机端分类链接的判断（684，687-691）
**/
function cat_pos($CAT, $str = ' &raquo; ', $target = '') {
	...
	foreach($arrparentid as $catid) {
		...
		if(strpos($UA, 'WINDOWS NT') !== false && !DT_DEBUG) {
			...
			$pos .= '<a href="http://wap.tecenet.com/'.$CATEGORY[$catid]['moduleid'].'-'.$CATEGORY[$catid]['catid'].'-0-0-1.html">'.$CATEGORY[$catid]['catname'].'</a>'.$str;			
		}
...
function captcha($captcha, $enable = 1, $return = false, $forward = 'goback', $wap=false) {
/*
* 修改日期2014-08-01
* 作者:陀梁生
* 修改函数captcha(),关联的函数为message()，关联template模板中的message文件下的wap_message.htm
* 修改说明:主要加多一个参数$wap,用于判断是否属于手机版，默认为false，当$wap为true，加载wap_message.htm文件
*/
	...
	if($enable) {
		if($DT['captcha_cn']) {
			if(strlen($captcha) < 4) {
				...
				return $return ? $msg : message($msg,$forward,$enable,$wap);
			}
		} else {
...
function mobileurl($moduleid, $catid = 0, $itemid = 0, $page = 1, $action = 0) {//修改伪静态生成函数
	if(RE_WRITE) return $moduleid.'-'.$catid.'-'.$itemid.'-'.$page.'-'.$action.'.html';
	if($itemid) {
		return 'index.php?moduleid='.$moduleid.'&itemid='.$itemid.($page > 1 ? '&page='.$page : '').'&action='.$action;
	} else if($catid) {
		return 'index.php?moduleid='.$moduleid.'&catid='.$catid.($page > 1 ? '&page='.$page : '').'&action='.$action;
	} else {
		return 'index.php?moduleid='.$moduleid.($page > 1 ? '&page='.$page : '').'&action='.$action;
	}
}
...
function itemurl($item, $page = 0) {
	...
	if(strpos($url, 'cat') !== false && $catid) {
		if(isset($item['gid'])) {//V5.0 升级 V6.0添加
			$catid = $item['gid'];
...
function log_write($message, $type = 'php', $force = 0) {//V5.0 升级 V6.0 修改
...
/*日期:2014-09-03
*添加自定义变量$html
*作用:更新自定义的HTML静态文件时,需要的是返回字符串,而不是直接输出
*关联的函数load();
*/
function ad($id, $cid = 0, $kw = '', $tid = 0, $html=false) {
	global $cityid;
	if($tid) {
		if($kw) {
	...
	if(is_file(DT_CACHE.'/htm/'.$a1)) return load($a1,$html);
	$a2 = $file.'_0.htm';
	if(is_file(DT_CACHE.'/htm/'.$a2)) return load($a2,$html);
	if(is_file(DT_CACHE.'/htm/'.$a3)) return load($a3,$html);
}
...
function dcurl($url, $par = '') {//V5.0 升级 V6.0添加
...
include DT_ROOT.'/include/tece.func.php';

//加载生成图片的函数
include DT_ROOT.'/include/captcha.php';
?>