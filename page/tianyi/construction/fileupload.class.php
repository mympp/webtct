...
class FileUpload {
    ...
    function upload($file) {
        ...
        /* 将文件上传的信息取出赋给变量 */
        $name = $file['name']; //上传文件的文件名 'Chrysanthemum.jpg'
        $tmp_name = $file['tmp_name']; //服务器端临时文件存放路径 'D:\wamp\tmp\php71E6.tmp'
        $size = $file['size']; //上传文件的大小 879394   858 KB (879,394 字节)
        $error = $file['error']; //错误信息 0
    ...
    private function setFiles($name="", $tmp_name="", $size=0, $error=0) {
        ...
        $aryStr = explode(".", $name);
        var_dump(strtolower($aryStr[count($aryStr)-1]));
        die;
    ...
    private function checkFileType() {
        ...
        if (in_array(strtolower($this->fileType), $this->allowtype)) {
            ...
            var_dump('================');
            var_dump(strtolower($this->fileType).'=====fileType=====<br>', $this->allowtype);
