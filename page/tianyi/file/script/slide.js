...
function dslide(id, time) {
	...
	this.htm += '<div style="width:'+(this.w-8)+'px;height:30px;overflow:hidden;z-index:3;position:relative;margin-top:-40px;padding:6px 4px 0 4px;text-align:center;">';
	for(var i = 0; i < this.max; i++) {
		this.htm += '<span id="'+id+'_no_'+i+'" style="padding:1px 5px 1px 5px;border-radius:10px;margin-left:5px;cursor:pointer;font-size:12px;color:#FFFFFF;'+(i == this.c ? 'background:#5FBC31;' : 'background:#D7D8D9;')+'">'+(i+1)+'</span>';
	}
	this.htm += '</div>';
	$('#'+id).html(this.htm);
	...
	this.slide = function(o) {
		...
		$('#'+id+'_no_'+this.c).css({background:'#D7D7D7'});
		$('#'+id+'_no_'+o).css({background:'#5FBC31'});
