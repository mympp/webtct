...
layer.photos = function(options){
    ...
    if(json){
        ...
        log.thistitle=nowimg.attr('data-title');
        log.thiscontent=nowimg.attr('data-content');
    ...
    var conf = {
        ...
        bgcolor: '',
        page: {
            html: '<div class="xubox_bigimg" style="overflow:scroll;position:relative;"><div class="xubox_bigimgtitle" style="font-size: 24px;margin-top: 30px;position: relative;">'+ function() {
                if(typeof (log.thistitle)!='undefined'){
                    return log.thistitle;
                }else{
                    return '';
                }
            }()+'</div><img src="'+ log.thissrc +'" alt="'+ (log.name || '') +'" layer-pid="'+ (log.pid || '') +'"><div class="xubox_bigimgcontent">'+ function() {
                if (typeof(log.thiscontent) != 'undefined'){
                    return log.thiscontent;
                }else{
                    return '';
                }
            }()+'</div><div class="xubox_imgsee">'+ function(){
                if(log.imgLen > 1){
                    return '<a href="" class="xubox_iconext xubox_prev"></a><a href="" class="xubox_iconext xubox_next"></a>'
                } else {
                    return '';
                }
            }() +'</div></div><div class="c_b"></div>'+ function(){
                if(options.html){
            ...
            log.timerr = setTimeout(function(){
                $('html').attr('layer-full', log.index);
            }, 10);
    ...
    log.event = function(){
        ...
        log.tabimg = function(){
            var timer, src, pid, name,imgtitle,imgcontent;
            ...
            if(json){
                ...
                imgtitle = thisimg.attr('data-title') || '';
                imgcontent = thisimg.attr('data-content') || '';
            ...
            $('.xubox_bigimgtitle').html(imgtitle);
            $('.xubox_bigimgcontent').html(imgcontent);
    ...
    log.resize = function(layero){
        ...
        //}
        var page_height=log.imgs[0].height+120+$('.xubox_bigimgcontent')[0].clientHeight;
        if(win.height()<page_height){
            page_height=win.height()-30;
        }
        $('.xubox_bigimgcontent').css({
            'padding-left':'50px',
            'padding-right':'50px',
            'padding-top':'20px',
            'margin':'0 auto',
        ...
        log.imgs.css({'margin-top': '30px'});
        log.imgs.css({visibility: 'visible'});
        log.bigimg.css({width: relog.flwidth, height: (page_height), 'background-color': options.bgcolor});
        if(options.html){
...
layer.photosPage = function(options){
    ...
    //console.log("$(options.parent).find('img')");
    //console.log($(options.parent).find('img')[1]);
    //console.log("$(options.parent).find('img')[1]");
