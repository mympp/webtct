...
$table = $DT_PRE.'mall';
$memtable = $DT_PRE.'member';
$typetable = $DT_PRE.'type';
$table_data = $DT_PRE.'mall_data';
//获取自定义分类
$uinfo = $db->get_one("SELECT userid FROM {$memtable} WHERE username='{$username}'");
$typewhere = 'mall-'.$uinfo['userid'];
$myc = $db->query("SELECT * FROM {$typetable} WHERE item='{$typewhere}' ORDER BY parentid asc,listorder asc");
$ci = 0;
while($r = $db->fetch_array($myc)) {
	$mycats[$ci]['typeid'] = $r['typeid'];
	$mycats[$ci]['parentid'] = $r['parentid'];
	$mycats[$ci]['typename'] = $r['typename'];
	$ci++;
}
if($itemid) {
	...
	if($items) {
		...
		while($r = $db->fetch_array($result)) {
			$r['alt'] = $r['title'];
			$r['img'] = str_replace('.thumb.jpg','',$r['thumb']);
