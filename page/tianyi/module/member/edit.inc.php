...
if($submit) {
	...
	if($endtime) $post['endtime']=strtotime($endtime);
	if($starttime) $post['starttime']=strtotime($starttime);
	if($retime) $post['retime']=strtotime($retime);
	if($endtime>$starttime){
		$post = dstripslashes($post);
		$post_check = array();
		if($_E) {
			if(in_array('thumb', $_E) || in_array('content', $_E)) clear_upload($post['thumb'].$post['content'].$post['image'], $_userid);
			foreach($_E as $k) {
				if($post[$k] != $user[$k]) {
					$post_check[$k] = $post[$k];
					$post[$k] = $user[$k];
		...
		message('公司营业期限不合法开始时间大于结束时间，请修改');
	}
} else {
	...
	extract($user);
	$starttime==0?$starttime='':$starttime=date("Y-m-d",$starttime);
	$endtime==0?$endtime='':$endtime=date("Y-m-d",$endtime);
	$retime==0?$retime='':$retime=date("Y-m-d",$retime);
