...
class tycase {
    ...
    function tycase() {
		global $db, $DT_PRE;
		$this->table = $DT_PRE.'tycase';
		$this->db = &$db;
		$this->fields = array('itemid','companyid','username','title', 'content','thumb','addtime','edittime','editor','totime','hits','status');
    }

	function pass($post) {
		global $DT_TIME, $L;
		if(!is_array($post)) return false;
		if(!$post['title']) return $this->_($L['tycase_pass_title']);
		if(!$post['thumb']) return $this->_($L['tycase_pass_thumb']);
		if($post['totime']) {
			if(!is_date($post['totime'])) return $this->_($L['tycase_pass_todate']);
			if(datetotime($post['totime'].' 23:59:59') < $DT_TIME) return $this->_($L['tycase_pass_todate_error']);
		}
