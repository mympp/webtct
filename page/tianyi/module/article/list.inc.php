...
if($items) {
	...
	while($r = $db->fetch_array($result)) {
		$r['adddate'] = timetodate($r['addtime'], 7);
		$r['editdate'] = timetodate($r['edittime'], 7);
...
$hot_list=tag("moduleid=16&template=null&offset=0&condition=elite=1&pagesize=3");
foreach($hot_list as &$one){
	$nopic = DT_SKIN.'image/nopic240.gif';
	$one['thumb'] = $one['thumb'] ? str_replace('.thumb.', '.middle.', $one['thumb']) : $nopic;
}
